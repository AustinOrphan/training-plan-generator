...
// Missing type definitions that are used in this file
interface MediumLongRunPattern {
  name: string;
  description: string;
...
interface ThresholdVolumeProgression {
  week: number;
  volume: number;
  pace: string;
...
interface TaperIntegration {
  weeks: number;
  volumeReduction: number;
}

interface TuneUpRace {
  weeksBeforeTarget: number;
  distance: string;
  effort: string;
}
...
export type PfitzingerTrainingPaces = Record<string, PaceRangeWithTarget>;
...
export interface PaceRangeWithTarget {
  min: number;
  max: number;
  target: number;
}
...
export abstract class BaseTrainingPhilosophy implements TrainingPhilosophy {
   ...
   */
  enhancePlan(basePlan: TrainingPlan): TrainingPlan {
    const enhancedBlocks = basePlan.blocks.map((block) =>
   ...
   */
  getPhaseIntensityDistribution(phase: TrainingPhase): IntensityDistribution {
    const methodologyDistributions =
      METHODOLOGY_INTENSITY_DISTRIBUTIONS[
        this.methodology as keyof typeof METHODOLOGY_INTENSITY_DISTRIBUTIONS
      ];
  ...
  getWorkoutEmphasis(type: WorkoutType): number {
    const emphasis =
      WORKOUT_EMPHASIS[this.methodology as keyof typeof WORKOUT_EMPHASIS];
  ...
  protected adjustIntensity(
    ...
    switch (phase) {
      ...
      case "peak":
        ...
        break;
      case "taper":
        adjustment = 0.9; // Reduced intensity in taper
        break;
      case "recovery":
...
export class PhilosophyFactory {
  private static philosophyCache = new Map<
    TrainingMethodology,
    TrainingPhilosophy
  >();
...
class DanielsPhilosophy extends BaseTrainingPhilosophy {
  ...
  enhancePlan(basePlan: TrainingPlan): TrainingPlan {
    ...
    const currentVDOT =
      basePlan.config.currentFitness?.vdot ||
      this.estimateVDOTFromPlan(basePlan);
    ...
    const danielsCompliantPlan =
      this.validateAndEnforceIntensityDistribution(pacedPlan);
    ...
    return {
      ...
      ...({
        metadata: {
          ...(danielsCompliantPlan as any).metadata,
  ...
  customizeWorkout(
    ...
    const danielsSegments = baseCustomization.segments.map((segment) =>
      this.customizeSegmentWithVDOTPaces(
        segment,
        template.type,
        phase,
        trainingPaces,
    ...
    return {
      ...
      ...({
        metadata: {
          ...(baseCustomization as any).metadata,
          vdot: currentVDOT,
          trainingPaces,
          methodology: "daniels",
        },
  ...
  private customizeSegmentWithVDOTPaces(
    ...
    const paceRange: PaceRange = {
      min: targetPace * 0.98, // 2% faster
      max: targetPace * 1.02, // 2% slower
    };
   ...
   */
  private getPhaseIntensityAdjustment(
    phase: TrainingPhase,
    paceZone: keyof DanielsTrainingPaces,
  ): number {
    const adjustments: Record<
      TrainingPhase,
      Record<keyof DanielsTrainingPaces, number>
    > = {
      base: {
   ...
   */
  private formatPaceDescription(
    pace: PaceRange,
    zone: keyof DanielsTrainingPaces,
  ): string {
    ...
    const target = formatPace((pace.min + pace.max) / 2); // Calculate target as midpoint
  ...
  private estimateVDOTFromTemplate(template: Workout): number {
    ...
    const avgIntensity =
      template.segments.reduce((sum, seg) => sum + (seg.intensity || 75), 0) /
   ...
   */
  private selectBasePhaseWorkout(
    type: WorkoutType,
    weekInPhase: number,
    templates: string[],
  ): string {
    switch (type) {
      ...
      case "threshold":
        ...
        if (weekInPhase <= 4) {
          return templates.includes("TEMPO_CONTINUOUS")
            ? "TEMPO_CONTINUOUS"
            : templates[0];
        }
   ...
   */
  private selectBuildPhaseWorkout(
    type: WorkoutType,
    weekInPhase: number,
    templates: string[],
  ): string {
   ...
   */
  private selectPeakPhaseWorkout(
    type: WorkoutType,
    weekInPhase: number,
    templates: string[],
  ): string {
    switch (type) {
      ...
      case "threshold":
        ...
        return templates.includes("THRESHOLD_PROGRESSION")
          ? "THRESHOLD_PROGRESSION"
          : this.selectThresholdWorkout(weekInPhase, templates);
   ...
   */
  private selectTaperPhaseWorkout(
    type: WorkoutType,
    weekInPhase: number,
    templates: string[],
  ): string {
   ...
   */
  private selectRecoveryPhaseWorkout(
    type: WorkoutType,
    templates: string[],
  ): string {
    switch (type) {
   ...
   */
  private selectThresholdWorkout(
    weekInPhase: number,
    templates: string[],
  ): string {
   ...
   */
  calculateVDOTFromRuns(runs: RunData[]): {
    vdot: number;
   ...
   */
  private getDanielsSpecificPace(
    zoneName: string,
    paces: DanielsTrainingPaces,
   ...
   */
  private calculateVDOTHeartRates(
    zoneName: string,
    vdot: number,
    ...
    switch (zoneName) {
      ...
      case "Neuromuscular":
        return {
          ...
          max: Math.round(estimatedMaxHR * 1.0),
        };
      default:
        return {
   ...
   */
  private enhanceSegmentWithVDOTInfo(
    description: string,
    zoneName: string,
    paces: DanielsTrainingPaces,
  ): string {
   ...
   */
  private generatePaceRecommendations(
    paces: DanielsTrainingPaces,
  ...
  private calculateActualIntensityDistribution(
    ...
    if (totalMinutes === 0)
      return { easy: 80, moderate: 15, hard: 5, veryHard: 0 }; // Default
  ...
  private adjustWorkoutsToTargetDistribution(
    ...
    const currentDistribution =
      this.calculateWorkoutGroupDistribution(workouts);
   ...
   */
  private calculateWorkoutGroupDistribution(
    workouts: PlannedWorkout[],
  ): IntensityDistribution {
    let easyMinutes = 0;
    let moderateMinutes = 0;
  ...
  private applyDistributionAdjustments(
    ...
    if (adjustment.hard > 0 && phase !== "base") {
      const eligibleWorkouts = adjustedWorkouts.filter((w) =>
        this.canAddQuality(w),
      );
  ...
  private canConvertToEasier(workout: PlannedWorkout): boolean {
    ...
    return (
      hasModerateIntensity &&
      workout.workout.type !== "threshold" &&
      workout.workout.type !== "vo2max"
    );
  }
  ...
  private validateIntensityDistribution(plan: TrainingPlan): {
    ...
    plan.blocks.forEach((block) => {
      ...
      phaseDistributions[`${block.phase}-${block.startDate}`] =
        phaseDistribution;
    ...
    const overallTarget = {
      ...
      veryHard: 0, // Add missing veryHard property
    } as IntensityDistribution; // Daniels uses polarized model
  ...
  private autoAdjustIntensityDistribution(
    ...
    if (
      ...
      revalidation.violations.length < violations.length
    ) {
  ...
  private reduceWorkoutIntensity(
    ...
    if (
      !this.shouldAdjustWorkout(workout, violation) ||
    ...
    const modifiedSegments = workout.workout.segments.map((segment) => {
      ...
      if (segment.intensity > 85) {
        ...
        return {
          ...
          zone:
            newIntensity <= 75 ? TRAINING_ZONES.EASY : TRAINING_ZONES.STEADY,
          description: this.updateSegmentDescription(
            segment.description,
            "reduced",
          ),
  ...
  private shouldAdjustWorkout(
    ...
    if (
      workout.workout.type === "race_pace" ||
      workout.workout.type === "time_trial"
    ) {
   ...
   */
  private updateSegmentDescription(
    description: string,
    adjustment: "easier" | "reduced",
  ): string {
   ...
   */
  private adjustIntensityForDaniels(
    baseIntensity: number,
    workoutType: WorkoutType,
    phase: TrainingPhase,
  ): number {
    ...
    switch (workoutType) {
      ...
      case "speed":
        ...
        adjustment = 1.0; // Keep at template intensity
        break;
      default:
        adjustment = 1.0;
    }
   ...
   */
  private enhanceSegmentDescription(
    baseDescription: string,
    workoutType: WorkoutType,
  ): string {
  ...
  private getDanielsAdaptationTarget(
    ...
    const adaptationTargets: Partial<
      Record<WorkoutType, Record<TrainingPhase, string>>
    > = {
      easy: {
    ...
    return (
      adaptationTargets[workoutType]?.[phase] ||
...
class AerobicBaseCalculator {
   ...
   */
  private convertToAerobicBase(
    workouts: PlannedWorkout[],
    currentDistribution: IntensityDistribution,
  ): PlannedWorkout[] {
    ...
    for (let i = 0; i < sortedWorkouts.length && remainingToConvert > 0; i++) {
      ...
      if (currentIntensity > 75) {
        // Only convert non-easy workouts
   ...
   */
  private convertToEffortDescription(
    description: string,
    intensity: number,
  ): string {
    ...
    if (intensity <= 70) {
      ...
      effortLevel =
        "Moderate effort - controlled discomfort, rhythmic breathing";
    } else {
  ...
  calculateLongRunProgression(
    ...
    const progressionRate =
      (targetDistance - baseDistance) / (totalWeeks * 0.7); // Build over 70% of base phase
...
class LydiardHillGenerator {
   ...
   */
  private getHillProfileForPhase(
    phase: TrainingPhase,
    weekInPhase: number,
  ): LydiardHillProfile {
    switch (phase) {
      case "base":
        return {
          ...
          adaptationTarget:
            "Leg strength, running economy, aerobic power development",
          principles: [
      ...
      case "build":
        return {
          ...
          adaptationTarget:
            "Anaerobic power, lactate tolerance, neuromuscular coordination",
          principles: [
      ...
      case "peak":
        return {
          ...
          adaptationTarget:
            "Speed coordination, neuromuscular power, race preparation",
          principles: [
   ...
   */
  private createHillSegments(
    profile: LydiardHillProfile,
    totalDuration: number,
  ): WorkoutSegment[] {
  ...
  private calculateHillTSS(segments: WorkoutSegment[]): number {
    ...
    segments.forEach((segment) => {
      ...
      const segmentTSS =
        (segment.duration * Math.pow(intensityFactor, 2) * 100) / 60;
...
class LydiardPeriodizationSystem {
  ...
  createLydiardBlocks(config: TrainingPlanConfig): TrainingBlock[] {
    // Calculate total weeks from start to end/target date
    const endDate = config.endDate || config.targetDate;
    const totalWeeks = endDate
      ? Math.ceil(
          (endDate.getTime() - config.startDate.getTime()) /
            (7 * 24 * 60 * 60 * 1000),
        )
    ...
    const durations = this.calculatePhaseDurations(
      totalWeeks,
      "half-marathon", // Use default race type
    );
    ...
    blocks.push({
      ...
      phase: "base",
      startDate: new Date(
        config.startDate.getTime() + (weekNumber - 1) * 7 * 24 * 60 * 60 * 1000,
      ),
      endDate: new Date(
        config.startDate.getTime() +
          (weekNumber + durations.aerobicBase - 2) * 7 * 24 * 60 * 60 * 1000,
      ),
    ...
    blocks.push({
      ...
      endDate: new Date(
        config.startDate.getTime() +
          (weekNumber + durations.hillPhase - 2) * 7 * 24 * 60 * 60 * 1000,
      ),
    ...
    blocks.push({
      ...
      endDate: new Date(
        config.startDate.getTime() +
          (weekNumber + durations.anaerobic - 2) * 7 * 24 * 60 * 60 * 1000,
      ),
    ...
    blocks.push({
      ...
      endDate: new Date(
        config.startDate.getTime() +
          (weekNumber + durations.coordination - 2) * 7 * 24 * 60 * 60 * 1000,
      ),
    ...
    blocks.push({
      ...
      endDate: new Date(
        config.startDate.getTime() +
          (weekNumber + durations.taper - 2) * 7 * 24 * 60 * 60 * 1000,
      ),
   ...
   */
  private getLydiardWeeklyWorkouts(
    phase: TrainingPhase,
    weekInPhase: number,
  ...
  validatePhaseTransition(
    ...
    const recommendations = isReady
      ? []
      : ["Complete more base work before progressing"];
...
class LydiardPhilosophy extends BaseTrainingPhilosophy {
  ...
  enhancePlan(basePlan: TrainingPlan): TrainingPlan {
    ...
    // Apply Lydiard periodization system using plan summary
    const totalWeeks = basePlan.summary?.totalWeeks || 12;
    // Target race would be in advanced config if available
    // Check if config has targetRaces (from AdvancedPlanConfig)
    const advancedConfig = basePlan.config as any;
    ...
    if (targetRace) {
      ...
      const lydiardBlocks = this.periodizationSystem.createLydiardBlocks(
        basePlan.config,
      );
    ...
    const aerobicCompliantWorkouts =
      this.aerobicBaseCalculator.enforceAerobicBase(enhancedPlan.workouts);
    ...
    const recoveryEmphasizedWorkouts = timeBasedWorkouts.map(
      (plannedWorkout) => {
        ...
        const weekNumber =
          Math.floor(
            (plannedWorkout.date.getTime() -
              timeBasedWorkouts[0].date.getTime()) /
              (7 * 24 * 60 * 60 * 1000),
          ) + 1;
        const isRecoveryWeek = weekNumber % 4 === 0;

        // Get the training phase from the workout or default to 'base'
        const phase = (plannedWorkout as any).phase || "base";
      ...
      ...({
        metadata: {
          ...
          lydiardFeatures: {
            ...
            longRunProgression:
              this.calculateLongRunProgressionForPlan(enhancedPlan),
  ...
  private applyLydiardPhaseEmphasis(block: TrainingBlock): TrainingBlock {
    const enhancedMicrocycles = block.microcycles.map((microcycle) => {
      ...
      const modifiedWorkouts = microcycle.workouts.map((plannedWorkout) => {
        ...
        if (
          block.phase === "base" &&
          this.isHighIntensityWorkout(plannedWorkout.workout.type)
        ) {
  ...
  private adjustIntensityForLydiard(
    ...
    switch (workoutType) {
      ...
      case "vo2max":
        // Minimal VO2max work
        if (phase === "base" || phase === "build") {
          adjustment = 0.8; // Convert to aerobic work
        } else {
          adjustment = 1.0;
        }
        break;
   ...
   */
  private getLydiardPhaseDistribution(
    phase: TrainingPhase,
  ): IntensityDistribution {
    switch (phase) {
      case "base":
        return { easy: 95, moderate: 4, hard: 1, veryHard: 0 }; // Extreme aerobic emphasis
      case "build":
        return { easy: 90, moderate: 8, hard: 2, veryHard: 0 }; // Still very aerobic
      case "peak":
        return { easy: 85, moderate: 10, hard: 5, veryHard: 0 }; // Some intensity added
      case "taper":
        return { easy: 92, moderate: 5, hard: 3, veryHard: 0 }; // Back to easy emphasis
      case "recovery":
        return { easy: 98, moderate: 2, hard: 0, veryHard: 0 }; // Almost all easy
      default:
   ...
   */
  private enhanceLydiardDescription(
    baseDescription: string,
    workoutType: WorkoutType,
    phase: TrainingPhase,
  ): string {
    const lydiardTerms: Partial<
      Record<WorkoutType, Record<TrainingPhase, string>>
    > = {
      easy: {
  ...
  private getLydiardAdaptationTarget(
    ...
    const adaptationMatrix: Partial<
      Record<WorkoutType, Record<TrainingPhase, string>>
    > = {
      easy: {
    ...
    return (
      adaptationMatrix[workoutType]?.[phase] ||
   ...
   */
  private calculateLongRunProgressionForPlan(
    plan: TrainingPlan,
  ): LongRunProgression {
    ...
    const currentLongestDistance =
      currentLongRuns.length > 0
        ? Math.max(
      ...
      for (let week = 1; week <= block.weeks; week++) {
        ...
        if (block.phase === "base") {
          ...
          const progressedDistance =
            this.aerobicBaseCalculator.calculateLongRunProgression(
              weekCounter,
              totalBaseWeeks,
          ...
          progression.push({
            ...
            effort:
              block.phase === "peak"
                ? "Moderate with race pace segments"
                : "Easy aerobic",
            focus:
              block.phase === "peak"
                ? "Race preparation"
   ...
   */
  private selectLydiardHillWorkout(
    phase: TrainingPhase,
    weekInPhase: number,
  ): string {
    // Generate phase-specific Lydiard hill templates using the hill generator
    const hillTemplates =
      this.lydiardHillGenerator.createLydiardHillTemplates();
...
class PfitsingerPhilosophy extends BaseTrainingPhilosophy {
  ...
  enhancePlan(basePlan: TrainingPlan): TrainingPlan {
    ...
    const planWithProgression =
      this.applyThresholdVolumeProgression(ltBasedPlan);
    ...
    return {
      ...
      ...({
        metadata: {
          ...(planWithRaces as any).metadata,
          methodology: "pfitzinger",
          lactateThresholdPace: ltPace,
          pfitzingerPaces,
          thresholdVolumeProgression:
            this.calculateLegacyThresholdVolumeProgression(planWithRaces),
  ...
  customizeWorkout(
    ...
    if (this.isMarathonSpecificWorkout(template.type, phase)) {
      pfitzingerSegments.forEach((segment) => {
        if (
          segment.zone.name === "Tempo" ||
          segment.zone.name === "Threshold"
        ) {
  ...
  selectWorkout(
    ...
    switch (type) {
      ...
      case "vo2max":
        ...
        if (phase === "peak") {
          return availableTemplates.includes("VO2MAX_5X3")
            ? "VO2MAX_5X3"
            : availableTemplates[0];
        }
    ...
    const enhancedMicrocycles = block.microcycles.map(
      (microcycle, weekIndex) => {
        const modifiedWorkouts = microcycle.workouts.map((plannedWorkout) => {
          ...
          if (
            plannedWorkout.workout.type === "long_run" &&
            this.shouldAddQuality(block.phase, weekIndex)
          ) {
        ...
        return {
          ...
          workouts: modifiedWorkouts,
          pattern:
            this.getPfitzingerWeeklyPattern(block.phase, weekIndex) ||
  ...
  private integrateRacesAndMediumLongs(plan: TrainingPlan): TrainingPlan {
    const modifiedWorkouts = plan.workouts.map((workout, index) => {
      const weeksToGoal = differenceInWeeks(
        plan.config.targetDate || plan.config.endDate || new Date(),
        workout.date,
      );

      // Add tune-up races at strategic points
      if (
        this.shouldBeRace(weeksToGoal) &&
        workout.workout.type === "long_run"
      ) {
   ...
   */
  private generateMediumLongRun(
    baseWorkout: PlannedWorkout,
    phase: TrainingPhase,
    weekNumber: number = 1,
  ): {
    name: string;
   ...
   */
  private selectMediumLongPattern(
    phase: TrainingPhase,
    weekNumber: number,
    ...
    if (phase === "base") {
      ...
      return weekNumber % 2 === 0
        ? patterns.raceSpecificMediumLong
        : patterns.tempoMediumLong;
    } else {
  ...
  private getMediumLongPatterns(): Record<string, MediumLongRunPattern> {
    return {
      ...
      aerobicMediumLong: {
        ...
        adaptationTarget:
          "Aerobic capacity, mitochondrial density, fat oxidation",
        estimatedTSS: 75,
        recoveryTime: 18,
      },
      ...
      tempoMediumLong: {
        ...
        description:
          "14-15 mile run with 4-6 mile tempo segment (signature Pfitzinger workout)",
      ...
      marathonPaceMediumLong: {
        ...
        description:
          "13-15 mile run with marathon pace segments for race simulation",
        ...
        adaptationTarget:
          "Marathon pace practice, pacing discipline, fatigue resistance",
        estimatedTSS: 95,
        recoveryTime: 20,
      },
      ...
      progressiveMediumLong: {
        ...
        description:
          "13-16 mile progressive run finishing at marathon pace or faster",
        ...
        adaptationTarget:
          "Progressive fatigue resistance, mental toughness, pace judgment",
        estimatedTSS: 110,
        recoveryTime: 26,
      },
      ...
      raceSpecificMediumLong: {
        ...
        adaptationTarget:
          "Race simulation, surge response, competitive fitness",
        estimatedTSS: 100,
        recoveryTime: 22,
      },
    };
   ...
   */
  private createMediumLongWorkout(
   ...
   */
  private generatePfitzingerWeeklyStructure(
    phase: TrainingPhase,
    weekIndex: number,
   ...
   */
  private getPfitzingerBaseStructure(
    phase: TrainingPhase,
  ): PfitzingerWeeklyStructure {
   ...
   */
  private calculateThresholdVolumeProgression(
    phase: TrainingPhase,
    weekIndex: number,
  ): ThresholdVolumeProgression {
    const baseStructure = this.getPfitzingerBaseStructure(phase);
    ...
    // Use baseVolume as proxy for threshold volume since thresholdVolume property doesn't exist
    let weeklyVolume =
      (baseStructure as any).thresholdVolume?.weeklyMinutes ||
      baseStructure.baseVolume * 0.2;

    // Progressive buildup in base and build phases
    if (phase === "base" || phase === "build") {
      const thresholdData = (baseStructure as any).thresholdVolume || {
        weeklyMinutes: baseStructure.baseVolume * 0.2,
        progressionRate: 1.1,
        maxVolume: baseStructure.baseVolume * 0.4,
      };
      weeklyVolume = Math.min(
        thresholdData.weeklyMinutes *
          Math.pow(thresholdData.progressionRate, Math.floor(currentWeek / 2)),
    ...
    return {
      ...
      pace: "6:00", // Fixed threshold pace per km (typical LT pace)
   ...
   */
  private distributeThresholdVolume(
    totalMinutes: number,
    phase: TrainingPhase,
   ...
   */
  private getThresholdIntensityTargets(
    phase: TrainingPhase,
  ): Record<string, number> {
   ...
   */
  private getThresholdRecoveryRequirements(
    weeklyVolume: number,
  ): RecoveryRequirements {
    return {
      days: weeklyVolume > 30 ? 2 : 1, // More recovery days at high volume
      intensity: Math.max(60, 70 - weeklyVolume * 0.2).toString() + "%", // Easier recovery as volume increases
    };
  }
   ...
   */
  private getRaceSpecificIntegration(
    phase: TrainingPhase,
    weekIndex: number,
    ...
    return {
      ...
      marathonPaceVolume:
        phase === "peak" ? 0.25 : phase === "build" ? 0.15 : 0.1,
    };
  }
   ...
   */
  private calculateMarathonPaceVolume(
    phase: TrainingPhase,
    weeksToRace: number,
  ): number {
   ...
   */
  private getRaceSimulationFrequency(
    phase: TrainingPhase,
    weeksToRace: number,
  ): number {
  ...
  private applyPfitzingerWeeklySpacing(
    ...
    Object.entries(volumeProgression.sessionDistribution).forEach(
      ([day, minutes]) => {
        if (
          enhancedStructure.dayStructure[
            day as keyof typeof enhancedStructure.dayStructure
          ]
        ) {
          enhancedStructure.dayStructure[
            day as keyof typeof enhancedStructure.dayStructure
    ...
    if (raceIntegration.marathonPaceVolume > 0) {
      ...
      ["wednesday", "saturday"].forEach((day) => {
        ...
        if (
          dayStructure &&
          (dayStructure.type === "medium_long" ||
            dayStructure.type === "long_quality")
        ) {
   ...
   */
  private getPfitzingerWeeklyPattern(
    phase: TrainingPhase,
    weekIndex: number,
  ): string | undefined {
   ...
   */
  private isMarathonSpecificWorkout(
    type: WorkoutType,
    phase: TrainingPhase,
  ): boolean {
    return (
      (type === "tempo" || type === "threshold" || type === "progression") &&
      (phase === "build" || phase === "peak")
    );
  }
   ...
   */
  private getMarathonPaceTarget(zoneName: string): {
    min: number;
    max: number;
  } {
   ...
   */
  private enhancePfitzingerDescription(
    baseDescription: string,
    workoutType: WorkoutType,
    phase: TrainingPhase,
  ): string {
   ...
   */
  private getPfitzingerAdaptationTarget(
    workoutType: WorkoutType,
    phase: TrainingPhase,
  ): string {
    const adaptationTargets: Partial<
   ...
   */
  private adjustTSSForPfitzinger(
    baseTSS: number,
    workoutType: WorkoutType,
  ): number {
    ...
    const multiplier = tssMultipliers[workoutType] || 1.0;
  ...
  private calculateLactateThresholdPace(plan: TrainingPlan): number {
    ...
    if (currentFitness?.recentRaces?.length) {
      ...
      // Create RunData object from race information
      const raceData: RunData = {
        date: new Date(),
        distance: distanceKm,
  ...
  private applyLTBasedPacing(
    ...
    return {
      ...
      workout: {
        ...
        ...({
          metadata: {
            ...(workout.workout as any).metadata,
  ...
  private applyThresholdVolumeProgression(plan: TrainingPlan): TrainingPlan {
    const totalWeeks =
      ...
      plan.blocks.reduce((sum, block) => sum + block.weeks, 0) ||
      12;
    const thresholdProgression =
      this.calculateLegacyThresholdVolumeProgression(plan);

    // Apply threshold volume to each week
    const enhancedWorkouts = plan.workouts.map((workout, index) => {
      const weekIndex = Math.floor(index / 7);
      const targetThresholdVolume =
        thresholdProgression[weekIndex] || thresholdProgression[0];
   ...
   */
  private calculateLegacyThresholdVolumeProgression(
    plan: TrainingPlan,
  ): number[] {
    const totalWeeks =
      plan.summary?.totalWeeks ||
    ...
    for (let week = 0; week < totalWeeks; week++) {
      ...
      if (week % 4 === 3) {
        ...
        weeklyVolume =
          baseThresholdVolume +
          (peakThresholdVolume - baseThresholdVolume) * progressRatio * 0.75;
      } else {
        // Build weeks: progressive increase
        weeklyVolume =
          baseThresholdVolume +
          (peakThresholdVolume - baseThresholdVolume) * progressRatio;
      }
   ...
   */
  private adjustWorkoutForThresholdVolume(
    workout: PlannedWorkout,
    targetVolume: number,
  ): PlannedWorkout {
    const thresholdSegments = workout.workout.segments.filter(
      (seg) => seg.intensity >= 84 && seg.intensity <= 92, // Threshold intensity range
    );
   ...
   */
  private getLTAdjustmentForZone(zoneName: string): {
    min: number;
    max: number;
  } {
  ...
  private getLTBasedDescription(zoneName: string, ltPace: number): string {
    ...
    return (
      descriptions[zoneName] || `Training pace based on LT pace (${paceStr})`
    );
  }
...
export const PhilosophyUtils = {
  ...
  comparePhilosophies(
    ...
    const intensityDiff = Math.abs(
      philosophy1.intensityDistribution.hard -
        philosophy2.intensityDistribution.hard,
    );
  ...
  recommendPhilosophy(characteristics: {
    ...
    if (
      characteristics.experience === "beginner" ||
      characteristics.injuryHistory
    ) {
    ...
    if (
      characteristics.goals === "race_performance" ||
      characteristics.goals === "competitive"
    ) {
