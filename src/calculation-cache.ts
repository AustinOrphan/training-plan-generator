...
export class CalculationProfiler {
  ...
  static profile<T>(operation: string, fn: () => T): T {
    ...
    this.metrics[operation].avgTime =
      this.metrics[operation].totalTime / this.metrics[operation].calls;
...
export class MemoryMonitor {
  ...
  static getMemoryIncrease(fromOperation: string, toOperation: string): number {
    ...
    return (
      (toSnapshot.memory.heapUsed - fromSnapshot.memory.heapUsed) /
      (1024 * 1024)
    ); // MB
  }
