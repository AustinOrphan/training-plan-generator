...
export class ValidationPipeline {
  ...
  validateConfig(config: unknown): ValidationResult {
    ...
    const configValidation = validationUtils.safeCast(
      config,
      validationGuards.isAdvancedPlanConfig,
      "config",
    );
    ...
    if (!primitiveGuards.isNonEmptyString(validConfig.name)) {
      this.addError(
        "config.name",
        "Plan name is required and must be non-empty",
      );
    }
    ...
    if (!primitiveGuards.isNonEmptyString(validConfig.goal)) {
      this.addError(
        "config.goal",
        "Training goal is required and must be non-empty",
      );
    }
    ...
    if (
      primitiveGuards.isDate(validConfig.startDate) &&
      primitiveGuards.isDate(validConfig.targetDate)
    ) {
      const duration =
        validConfig.targetDate.getTime() - validConfig.startDate.getTime();
      const weeks = duration / (7 * 24 * 60 * 60 * 1000);

      if (weeks < 4) {
        this.addError(
          "config.duration",
          "Plan duration must be at least 4 weeks",
        );
      }
      ...
      if (weeks > 52) {
        this.addWarning(
          "config.duration",
          "Plan duration exceeds 52 weeks - consider breaking into phases",
        );
      }
    ...
    const fitnessValidation = validationUtils.safeCast(
      validConfig.currentFitness,
      validationGuards.isFitnessAssessment,
      "currentFitness",
    );
    if (fitnessValidation.success) {
      ...
      this.addError(
        ...
        "Invalid fitness assessment: " + fitnessValidation.error.message,
      );
    }

    // Preferences validation - use type guard
    const preferencesValidation = validationUtils.safeCast(
      validConfig.preferences,
      validationGuards.isTrainingPreferences,
      "preferences",
    );
    if (preferencesValidation.success) {
      const preferences = preferencesValidation.data;
      if (
        primitiveGuards.isArray(
          preferences.availableDays,
          primitiveGuards.isNumber,
        )
      ) {
        const days = preferences.availableDays;
        if (days.length < 3) {
          this.addWarning(
            "config.preferences.availableDays",
            "Less than 3 training days may limit plan effectiveness",
          );
        }
        ...
        days.forEach((day) => {
          if (day < 0 || day > 6) {
            this.addError(
              "config.preferences.availableDays",
              `Invalid day value: ${day}. Must be 0-6`,
            );
          }
        });
      }
      ...
      if (primitiveGuards.isObject(preferences.timeConstraints)) {
        Object.entries(preferences.timeConstraints).forEach(([day, time]) => {
          ...
          if (isNaN(dayNum) || dayNum < 0 || dayNum > 6) {
            this.addError(
              "config.preferences.timeConstraints",
              `Invalid day key: ${day}`,
            );
          }
          if (primitiveGuards.isNumber(time)) {
            if (time < 15) {
              this.addError(
                "config.preferences.timeConstraints",
                `Time constraint too short: ${time} minutes on day ${day}`,
              );
            }
            if (time > 480) {
              this.addWarning(
                "config.preferences.timeConstraints",
                `Time constraint very long: ${time} minutes on day ${day}`,
              );
            }
          }
        });
      ...
      this.addError(
        "config.preferences",
        "Invalid training preferences: " + preferencesValidation.error.message,
      );
    }

    // Methodology validation using type-safe approach
    if (
      validConfig.methodology &&
      !["daniels", "lydiard", "pfitzinger", "hanson", "custom"].includes(
        validConfig.methodology,
      )
    ) {
      this.addError(
        "config.methodology",
        `Invalid methodology: ${validConfig.methodology}`,
      );
    }
  ...
  private validateFitnessAssessment(fitness: FitnessAssessment): void {
    if (fitness.vdot !== undefined) {
      if (fitness.vdot < 20 || fitness.vdot > 85) {
        this.addError(
          "currentFitness.vdot",
          `VDOT ${fitness.vdot} out of valid range (20-85)`,
        );
      }
    }
    ...
    if (fitness.weeklyMileage !== undefined) {
      if (fitness.weeklyMileage < 0) {
        this.addError(
          "currentFitness.weeklyMileage",
          "Weekly mileage cannot be negative",
        );
      }
      if (fitness.weeklyMileage > 200) {
        this.addWarning(
          "currentFitness.weeklyMileage",
          "Weekly mileage exceeds 200km - ensure this is accurate",
        );
      }
    }
    ...
    if (fitness.longestRecentRun !== undefined) {
      if (fitness.longestRecentRun < 0) {
        this.addError(
          "currentFitness.longestRecentRun",
          "Longest run cannot be negative",
        );
      }
      if (
        fitness.weeklyMileage &&
        fitness.longestRecentRun > fitness.weeklyMileage
      ) {
        this.addError(
          "currentFitness.longestRecentRun",
          "Longest run exceeds weekly mileage",
        );
      }
    }
    ...
    if (fitness.trainingAge !== undefined) {
      if (fitness.trainingAge < 0) {
        this.addError(
          "currentFitness.trainingAge",
          "Training age cannot be negative",
        );
      }
      if (fitness.trainingAge > 50) {
        this.addWarning(
          "currentFitness.trainingAge",
          "Training age exceeds 50 years - verify accuracy",
        );
      }
    }
    ...
    if (fitness.recoveryRate !== undefined) {
      if (fitness.recoveryRate < 0 || fitness.recoveryRate > 100) {
        this.addError(
          "currentFitness.recoveryRate",
          "Recovery rate must be between 0-100",
        );
      }
    }
  }
  ...
  validatePlan(plan: unknown): ValidationResult {
    ...
    const planValidation = validationUtils.safeCast(
      plan,
      validationGuards.isTrainingPlan,
      "plan",
    );
    ...
    if (primitiveGuards.isArray(validPlan.workouts)) {
      ...
      for (let i = 1; i < validPlan.workouts.length; i++) {
        if (
          validPlan.workouts[i].date.getTime() <
          validPlan.workouts[i - 1].date.getTime()
        ) {
          this.addError(
            "plan.workouts",
            `Workouts not in chronological order at index ${i}`,
          );
        }
      }
    }
    ...
    if (
      primitiveGuards.isObject(validPlan.summary) &&
      primitiveGuards.isArray(validPlan.workouts)
    ) {
      const actualWorkouts = validPlan.workouts.length;
      const summaryWorkouts = validPlan.summary.totalWorkouts;
      if (
        primitiveGuards.isNumber(summaryWorkouts) &&
        summaryWorkouts !== actualWorkouts
      ) {
        this.addError(
          "plan.summary.totalWorkouts",
          `Summary count (${summaryWorkouts}) doesn't match actual workouts (${actualWorkouts})`,
        );
      }
      ...
      const actualDistance = validPlan.workouts.reduce((sum, w) => {
        return (
          sum +
          (primitiveGuards.isNumber(w.targetMetrics.distance)
            ? w.targetMetrics.distance
            : 0)
        );
      }, 0);
      ...
      if (primitiveGuards.isNumber(summaryDistance)) {
        ...
        if (distanceDiff > 1) {
          ...
          this.addError(
            "plan.summary.totalDistance",
            `Summary distance (${summaryDistance}) doesn't match calculated (${actualDistance})`,
          );
        }
      }
    }
  ...
  private validateWorkout(
    ...
    if (!workout.date) {
      ...
      if (plan.config) {
        ...
        if (
          plan.config.targetDate &&
          workout.date.getTime() > plan.config.targetDate.getTime()
        ) {
    ...
    if (!workout.targetMetrics) {
      ...
      if (workout.targetMetrics.tss !== undefined) {
        if (workout.targetMetrics.tss < 0) {
          this.addError(
            `${prefix}.targetMetrics.tss`,
            "TSS cannot be negative",
          );
        }
  ...
  validateProgressData(progress: unknown): ValidationResult {
    ...
    const progressValidation = validationUtils.validateArray(
      progress,
    ...
    validProgress.forEach((entry, index) => {
      ...
      if (!primitiveGuards.isDate(entry.date)) {
        this.addError(
          `${prefix}.date`,
          "Progress date is required and must be valid",
        );
      }

      // Validate completion metrics - completedWorkouts is optional in ProgressData
      if (
        entry.completedWorkouts &&
        primitiveGuards.isArray(entry.completedWorkouts)
      ) {
        entry.completedWorkouts.forEach((completion, completionIndex) => {
          ...
          if (completion.completionRate < 0 || completion.completionRate > 1) {
            this.addError(
              `${completionPrefix}.completionRate`,
              "Completion rate must be between 0-1",
            );
          }

          if (completion.actualDuration < 0) {
            this.addError(
              `${completionPrefix}.actualDuration`,
              "Duration cannot be negative",
            );
          }

          if (
            completion.actualDistance !== undefined &&
            completion.actualDistance < 0
          ) {
            this.addError(
              `${completionPrefix}.actualDistance`,
              "Distance cannot be negative",
            );
          }
          ...
          if (!["complete", "partial", "none"].includes(completion.adherence)) {
            this.addError(
              `${completionPrefix}.adherence`,
              "Invalid adherence value",
            );
          }

          if (completion.difficultyRating !== undefined) {
            if (
              completion.difficultyRating < 1 ||
              completion.difficultyRating > 10
            ) {
              this.addError(
                `${completionPrefix}.difficultyRating`,
                "Difficulty rating must be 1-10",
              );
            }
          }
        });
  ...
  validateExportResult(
    ...
    if (!result.metadata?.format) {
      ...
      this.addError(
        ...
        `Format mismatch: expected ${expectedFormat}, got ${result.metadata.format}`,
      );
    }
    ...
    if (!result.filename) {
      ...
      if (!result.filename.endsWith(`.${expectedExt}`)) {
        this.addError(
          "result.filename",
          `Filename should end with .${expectedExt}`,
        );
      }
    }
      ...
      case "ical":
        if (typeof result.content === "string") {
          if (!result.content.includes("BEGIN:VCALENDAR")) {
            this.addError(
              "result.content",
              "Invalid iCal format - missing BEGIN:VCALENDAR",
            );
          }
          if (!result.content.includes("VERSION:2.0")) {
            this.addError(
              "result.content",
              "Invalid iCal format - missing VERSION:2.0",
            );
          }
        }
        break;
  ...
  validatePipelineConsistency(
        ...
        if (entry.completedWorkouts?.length > 0) {
          entry.completedWorkouts.forEach((workout) => {
            ...
            if (!found) {
              this.addError(
                `progress[${index}]`,
                `References non-existent workout ID: ${plannedId}`,
              );
            }
          });
        }
    ...
    if (plan && exportResult?.metadata) {
      if (exportResult.metadata.planName !== plan.config.name) {
        this.addWarning(
          "export.metadata",
          "Export plan name doesn't match plan",
        );
      }
      ...
      if (exportResult.metadata.totalWorkouts !== plan.workouts.length) {
        this.addError(
          "export.metadata",
          "Export workout count doesn't match plan",
        );
      }
    }
  ...
  static async validateWorkflow(
    ...
    if (exportResults) {
      for (const [format, result] of Object.entries(exportResults)) {
        results[`export_${format}`] = pipeline.validateExportResult(
          result,
          format,
        );
      }
    ...
    if (configValidation.success && planValidation.success) {
      const progressValidation = progress
