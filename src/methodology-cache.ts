...
export function generatePlanCached(
  ...
  // Only cache if configuration suggests it should be cached
  // For now, cache all results to improve performance
  // Could add specific caching conditions based on config properties later
...
export class MethodologyCacheWarmer {
  ...
  static async warmPhilosophyComparisons(
    comparator: (
      m1: TrainingMethodology,
      m2: TrainingMethodology,
    ) => MethodologyComparison,
  ): Promise<void> {
...
export class MethodologyCacheManager {
  ...
  static isPerformanceOptimal(): boolean {
    ...
    const avgHitRatio =
      Object.values(stats).reduce((sum, stat) => sum + stat.hitRatio, 0) /
  ...
  static getOptimizationRecommendations(): string[] {
    ...
    Object.entries(stats).forEach(([cacheName, stat]) => {
      if (stat.hitRatio < 0.5 && stat.metrics.misses > 10) {
        recommendations.push(
          `Low hit ratio for ${cacheName} cache (${(stat.hitRatio * 100).toFixed(1)}%) - consider increasing cache size`,
        );
      }

      if (stat.size === 0 && stat.metrics.misses > 0) {
        recommendations.push(
          `${cacheName} cache is empty but has misses - check cache warming`,
        );
      }
    });
    ...
    if (avgHitRatio < 0.7) {
      recommendations.push(
        `Overall cache hit ratio is low (${(avgHitRatio * 100).toFixed(1)}%) - consider cache warming strategy`,
      );
    }
