...
export class CustomWorkoutGenerator {
  ...
  public generateWorkout(
    ...
    const alternatives =
      compliance < 80 ? this.generateAlternatives(parameters) : undefined;
      ...
      case "hill_repeats":
        ...
        if (phase === "base") {
          ...
          segments.push({
            ...
            description:
              "Hill circuit: bound up, jog down, repeat continuously",
          });
        } else {
          // Interval hills
  ...
  private applyEnvironmentalModifications(
    ...
    if (
      ...
      environmentalFactors.altitude > 1500
    ) {
      const altitudeReduction = Math.min(
        5,
        Math.floor(environmentalFactors.altitude / 1000),
      );
      modifiedSegments = modifiedSegments.map((seg) => ({
        ...seg,
  ...
  private collectConstraintWarnings(
    ...
    if (
      parameters.environmentalFactors?.altitude &&
      parameters.environmentalFactors.altitude > 2500
    ) {
  ...
  private applyIntensityConstraints(
    ...
    if (
      environmentalFactors?.typicalTemperature &&
      environmentalFactors.typicalTemperature > 30
    ) {
  ...
  private getDefaultIntensity(type: WorkoutType, phase: TrainingPhase): number {
    const methodologyIntensities: Record<
      TrainingMethodology,
  ...
  private getZoneForIntensity(
    intensity: number,
  ): (typeof TRAINING_ZONES)[keyof typeof TRAINING_ZONES] {
