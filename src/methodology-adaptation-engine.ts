...
export class MethodologyAdaptationEngine extends SmartAdaptationEngine {
  private adaptationPatterns: Map<TrainingMethodology, MethodologyAdaptationPattern[]> = new Map();
  private responseProfiles: Map<string, MethodologyResponseProfile> = new Map();
  private philosophies: Map<TrainingMethodology, TrainingPhilosophy> = new Map();
  ...
  analyzeProgressWithMethodology(
    ...
    const baseProgress = super.analyzeProgress(completedWorkouts, plannedWorkouts);
  ...
  suggestMethodologyAwareModifications(
    ...
    const baseModifications = super.suggestModifications(plan, progress, recovery);
    ...
    if (!methodology) {
      return baseModifications.map(mod => this.convertToMethodologyModification(mod, 'custom'));
    }
    ...
    const allModifications = [
      ...baseModifications.map(mod => this.convertToMethodologyModification(mod, methodology)),
      ...methodologyModifications
    ];

    // Prioritize based on methodology principles and individual response history
    return this.prioritizeModifications(allModifications, methodology, plan.id);
  }
  ...
  updateResponseProfile(
    ...
    this.updateEffectivenessTrends(profile, modification, response.effectiveness);

    // Update preferred/avoided modifications
    this.updateModificationPreferences(profile, modification, response.effectiveness);
  ...
  private generateMethodologyInsights(
    ...
    return {
      ...
      keyMetrics: this.calculateMethodologyKeyMetrics(completedWorkouts, methodology),
      complianceScore: this.calculateMethodologyCompliance(completedWorkouts, plannedWorkouts, methodology)
    };
  }
          ...
          modifications: [
            {
              ...
              targetWorkouts: ['tempo', 'threshold', 'intervals'],
              adjustmentValue: -5, // 5% pace reduction
              duration: 14,
              methodologySpecific: true,
              philosophyPrinciple: 'VDOT-based pace prescription',
              confidence: 90
            } as MethodologyPlanModification
          ],
          ...
          modifications: [
            {
              ...
              targetWorkouts: ['intervals', 'tempo'],
              adjustmentValue: -15, // Reduce hard workouts
              duration: 10,
              methodologySpecific: true,
          ...
          modifications: [
            {
              ...
              targetWorkouts: ['easy', 'long'],
              adjustmentValue: 15, // 15% volume increase in easy runs
              duration: 21,
              methodologySpecific: true,
          ...
          modifications: [
            {
              ...
              targetWorkouts: ['threshold'],
              adjustmentValue: -25, // Replace some threshold with medium-long
              duration: 14,
              methodologySpecific: true,
    ...
    this.adaptationPatterns.set('hudson', [
      {
        ...
        response: {
          modifications: [
            {
              type: 'individualized_adjustment',
              reason: 'Adapt training based on individual response patterns',
              targetWorkouts: ['all'],
              adjustmentValue: 0, // Variable based on individual response
              duration: 7,
              methodologySpecific: true,
  ...
  private calculatePhilosophyAlignment(
    ...
    const easyAlignment = Math.abs(targetDistribution.easy - actualDistribution.easy);
    const moderateAlignment = Math.abs(targetDistribution.moderate - actualDistribution.moderate);
    const hardAlignment = Math.abs(targetDistribution.hard - actualDistribution.hard);
   ...
   */
  private calculateActualIntensityDistribution(completedWorkouts: CompletedWorkout[]): {
    easy: number;
    moderate: number;
    hard: number;
  } {
    ...
    const totalDuration = completedWorkouts.reduce((sum, w) => sum + (w.actualMetrics?.duration || 0), 0);
  ...
  private generateAdaptationRecommendations(
    ...
    if (completedWorkouts.length < 5) {
      recommendations.push('Complete more workouts to generate methodology-specific recommendations');
    ...
    switch (methodology) {
      case 'daniels':
        if (actualDistribution.hard > targetDistribution.hard + 5) {
          recommendations.push('Reduce hard training intensity to maintain 80/20 distribution');
        }
      ...
      case 'lydiard':
        if (actualDistribution.easy < targetDistribution.easy - 5) {
          recommendations.push('Increase aerobic base development with more easy running');
        }
        recommendations.push('Focus on time-based training rather than pace-specific work');
        break;

      case 'pfitzinger':
        const thresholdPercentage = this.calculateThresholdPercentage(completedWorkouts);
        if (thresholdPercentage > 15) {
          recommendations.push('Reduce lactate threshold volume to prevent overload');
        }
        recommendations.push('Incorporate medium-long runs with tempo segments');
        break;

      case 'hudson':
        recommendations.push('Monitor individual response and adjust training based on feedback');
        recommendations.push('Assess current adaptation and modify plan accordingly');
        break;

      case 'custom':
        recommendations.push('Monitor training balance and adjust based on personal response');
        break;
    }
  ...
  private detectVdotDecline(completedWorkouts: CompletedWorkout[], currentVdot: number): boolean {
    ...
    return recentWorkouts.some(w => w.actualMetrics?.perceivedExertion && w.actualMetrics.perceivedExertion > 8);
  }

  private calculateThresholdPercentage(completedWorkouts: CompletedWorkout[]): number {
    const totalDuration = completedWorkouts.reduce((sum, w) => sum + (w.actualDuration || 0), 0);
    const thresholdDuration = completedWorkouts
      .filter(w => w.plannedWorkout?.workout?.type && ['threshold', 'tempo'].includes(w.plannedWorkout.workout.type))
  ...
  private calculateIntensityBalance(completedWorkouts: CompletedWorkout[]): number { return 85; }
  private calculatePaceConsistency(completedWorkouts: CompletedWorkout[]): number { return 78; }
  private calculateAerobicVolume(completedWorkouts: CompletedWorkout[]): number { return 92; }
  private calculateTimeBasedCompliance(completedWorkouts: CompletedWorkout[]): number { return 88; }
  private calculateThresholdProgression(completedWorkouts: CompletedWorkout[]): number { return 83; }
  private calculateMediumLongFrequency(completedWorkouts: CompletedWorkout[]): number { return 75; }
  private calculateAdaptationRate(completedWorkouts: CompletedWorkout[]): number { return 80; }
  private calculateIndividualResponse(completedWorkouts: CompletedWorkout[]): number { return 85; }
  ...
  private calculateResponseEffectiveness(outcome: AdaptationResponse['outcomeMetrics']): number {
    const weights = {
      performance: 0.4,
      adherence: 0.3,
      recovery: 0.2,
  ...
  private updateModificationPreferences(
    ...
    if (effectiveness > 75) {
      ...
      const exists = profile.preferredModifications.some(pref => 
        pref.type === modification.type && pref.philosophyPrinciple === modification.philosophyPrinciple
      );
      if (!exists) {
      ...
      const exists = profile.avoidedModifications.some(avoid => 
        avoid.type === modification.type && avoid.philosophyPrinciple === modification.philosophyPrinciple
      );
      if (!exists) {
...
export const MethodologyAdaptationUtils = {
  ...
  analyzeAdaptationNeeds: (
    ...
    const result = engine.analyzeProgressWithMethodology(completedWorkouts, plannedWorkouts, plan);
