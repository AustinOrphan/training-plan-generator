...
export class MethodologyWorkoutSelector {
  ...
  public selectWorkout(
    ...
    if (
      criteria.workoutType === "tempo" ||
      criteria.workoutType === "threshold"
    ) {
  ...
  private applyContextModifications(
    ...
    if (
      criteria.equipment &&
      !criteria.equipment.includes("track") &&
      workout.type === "speed"
    ) {
    ...
    switch (this.methodology) {
      ...
      case "lydiard":
        ...
        const hardPercentage =
          hardSegments.reduce((sum, seg) => sum + seg.duration, 0) /
        ...
        if (
          criteria.phase === "base" &&
          ["vo2max", "speed"].includes(workout.type)
        ) {
  ...
  private checkPreferenceConflicts(
    ...
    if (
      preferences.preferredIntensity === "low" &&
  ...
  private adjustWorkoutDuration(
    ...
    if (difference > 0) {
      const maxSegmentIndex = adjustedSegments.reduce(
        (maxIdx, seg, idx, arr) =>
          seg.duration > arr[maxIdx].duration ? idx : maxIdx,
        0,
      );
   ...
   */
  private getZoneFromIntensity(intensity: number): TrainingZone {
  ...
  public getProgressedWorkout(
    ...
    if (
      ...
      previousWorkout.workout.segments &&
      previousWorkout.workout.segments.length > 0
    ) {
    ...
    if (
      performanceData &&
      performanceData.completionRate > 0.9 &&
      performanceData.difficultyRating <= 7
    ) {
  ...
  public selectRecoveryWorkout(
    ...
    if (recoveryNeeded > 80 && this.methodology === "lydiard") {
      ...
      return {
        ...
        rationale:
          "Lydiard methodology emphasizes complete rest for optimal recovery",
        methodologyCompliance: 100,
      };
    }
