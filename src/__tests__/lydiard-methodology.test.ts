...
describe('Lydiard Methodology Validation and Testing', () => {
  ...
  describe('Aerobic Base Emphasis Validation', () => {
    it('should enforce 85%+ easy aerobic running distribution', () => {
      ...
      const basePhaseDistribution = philosophy.getPhaseIntensityDistribution('base');
    ...
    it('should convert high-intensity workouts to aerobic alternatives in base phase', () => {
      ...
      const thresholdSelection = philosophy.selectWorkout('threshold', 'base', 4);
    ...
    it('should customize workouts with Lydiard principles', () => {
      ...
      const customized = philosophy.customizeWorkout(mockWorkout.workout, 'base', 1);
  ...
  describe('Hill Training Generation Accuracy', () => {
    it('should include Lydiard-specific hill repeat protocols', () => {
      ...
      const hillWorkoutBase = philosophy.selectWorkout('hill_repeats', 'base', 1);
      const hillWorkoutBuild = philosophy.selectWorkout('hill_repeats', 'build', 1);
      const hillWorkoutPeak = philosophy.selectWorkout('hill_repeats', 'peak', 1);
    ...
    it('should customize hill workouts with phase-specific intensities', () => {
      const mockHillWorkout = {
        ...
        segments: [
          ...
          { duration: 3, intensity: 90, zone: { name: 'VO2_MAX' }, description: 'Hill repeat' },
          { duration: 2, intensity: 50, zone: { name: 'Recovery' }, description: 'Recovery' }
        ],
      ...
      const baseCustomization = philosophy.customizeWorkout(mockHillWorkout, 'base', 2);
      expect(baseCustomization.adaptationTarget).toBeDefined();
      
      // Build phase customization
      const buildCustomization = philosophy.customizeWorkout(mockHillWorkout, 'build', 2);
      expect(buildCustomization.segments[1].intensity).toBeGreaterThan(0);
      
      // Peak phase customization
      const peakCustomization = philosophy.customizeWorkout(mockHillWorkout, 'peak', 2);
  ...
  describe('Periodization Progression Validation', () => {
    it('should validate phase transition requirements', () => {
      ...
      phases.forEach(phase => {
        const phaseDistribution = philosophy.getPhaseIntensityDistribution(phase);
        ...
        const total = phaseDistribution.easy + phaseDistribution.moderate + phaseDistribution.hard;
    ...
    it('should maintain appropriate intensity distribution across phases', () => {
      ...
      const buildDistribution = philosophy.getPhaseIntensityDistribution('build');
  ...
  describe('Workout Type Prioritization', () => {
    it('should prioritize steady-state runs, long runs, and hill training', () => {
      ...
      expect(philosophy.getWorkoutEmphasis('steady')).toBeGreaterThanOrEqual(1.0);
      expect(philosophy.getWorkoutEmphasis('hill_repeats')).toBeGreaterThanOrEqual(1.0);
      expect(philosophy.getWorkoutEmphasis('long_run')).toBeGreaterThanOrEqual(1.0);
    });
    ...
    it('should select appropriate workout types for each phase', () => {
      ...
      const workoutTypes: WorkoutType[] = ['easy', 'hill_repeats', 'tempo', 'threshold', 'long_run'];
  ...
  describe('Recovery Emphasis System', () => {
    ...
    it('should schedule appropriate recovery between hard efforts', () => {
      const hardWorkoutTypes: WorkoutType[] = ['hill_repeats', 'threshold', 'tempo'];
      ...
      hardWorkoutTypes.forEach(type => {
        const mockWorkout = {
          ...
          segments: [
            { duration: 20, intensity: 85, zone: { name: 'Threshold' }, description: 'Hard effort' }
          ],
  ...
  describe('Long Run Development', () => {
    it('should emphasize time on feet over speed in long runs', () => {
      ...
      const customized = philosophy.customizeWorkout(mockLongRun.workout, 'base', 5);
  ...
  describe('Integration and Edge Cases', () => {
    ...
    it('should maintain Lydiard principles across different workout types', () => {
      // Use workout types that have actual templates available
      const workoutTypes: WorkoutType[] = ['easy', 'long_run', 'hill_repeats', 'tempo', 'threshold'];
