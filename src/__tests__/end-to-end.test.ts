...
describe('End-to-End Integration', () => {
  describe('Complete Workflow Validation', () => {
    it('should execute full workflow from config to export', async () => {
      ...
      const firstWeekWorkouts = plan.workouts.filter(w => {
        const weekDiff = testDateUtils.calculateWeeks(plan.config.startDate, w.date);
      ...
      const progressData: ProgressData[] = plan.workouts.slice(0, 7).map((workout, index) => ({
        date: workout.date,
        completedWorkout: {
          plannedWorkout: workout,
          date: workout.date, // Add date property to completed workout
          actualDuration: workout.targetMetrics.duration * (0.9 + Math.random() * 0.2),
          actualDistance: (workout.targetMetrics.distance || 0) * (0.9 + Math.random() * 0.2),
      ...
      const modifications = adaptationEngine.suggestModifications(plan, latestProgress, latestProgress.recoveryMetrics);
  ...
  describe('Methodology Integration', () => {
    ...
    methodologies.forEach(methodology => {
      it(`should generate valid plan with ${methodology} methodology`, async () => {
        ...
        const easyWorkouts = (workoutCounts.easy || 0) + (workoutCounts.recovery || 0);
  ...
  describe('Validation Pipeline', () => {
    ...
    it('should validate data consistency throughout pipeline', () => {
      ...
      plan.workouts.forEach((workout, index) => {
        ...
        expect(workout.date.getTime()).toBeGreaterThanOrEqual(config.startDate.getTime());
        expect(workout.date.getTime()).toBeLessThanOrEqual(config.targetDate.getTime());
        
        // Workouts should be in chronological order
        if (index > 0) {
          expect(workout.date.getTime()).toBeGreaterThanOrEqual(plan.workouts[index - 1].date.getTime());
        }
      });
      ...
      // Summary consistency
      const expectedWeeks = testDateUtils.calculateWeeks(config.startDate, config.targetDate);
      ...
      expect(plan.summary.averageWeeklyDistance).toBeCloseTo(plan.summary.totalDistance / plan.summary.totalWeeks, 10); // Allow for floating point precision
      expect(plan.summary.peakWeeklyDistance).toBeGreaterThanOrEqual(plan.summary.averageWeeklyDistance);
    });
    ...
    it('should validate workout progression logic', () => {
      ...
      for (let week = 0; week < plan.summary.totalWeeks; week++) {
        const weekWorkouts = plan.workouts.filter(w => {
          const weekNum = testDateUtils.calculateWeeks(config.startDate, w.date);
      ...
      for (let i = 1; i < weeklyVolumes.length - 3; i++) { // Exclude taper weeks
      ...
      if (lastThreeWeeks.length > 0 && peakVolume > 0) {
        ...
        expect(lastThreeWeeks[lastThreeWeeks.length - 1]).toBeLessThan(peakVolume * 1.5); // Final week reasonable compared to peak
      }
    });
    ...
    it('should validate export data integrity', async () => {
      ...
      expect(exportedPlan.workouts || exportedPlan.data || exportedPlan.plan).toBeDefined();
      // Use flexible property access since the JSON structure may vary
      const actualWorkouts = exportedPlan.workouts || exportedPlan.data?.workouts || exportedPlan.plan?.workouts;
      if (actualWorkouts) {
  ...
  describe('Adaptive Planning Cycle', () => {
    it('should complete full adaptive planning cycle', async () => {
      ...
      for (const scenario of scenarios) {
        // Get week's workouts
        const { start: weekStart, end: weekEnd } = testDateUtils.createDateRange(config.startDate, scenario.week);
        ...
        const weekProgress: ProgressData[] = weekWorkouts.map(workout => ({
          ...
          completedWorkout: {
            plannedWorkout: workout,
            date: workout.date, // Add date property to completed workout
            actualDuration: workout.targetMetrics.duration * (scenario.performance === 'good' ? 1.0 : scenario.performance === 'poor' ? 0.7 : 0.85),
            actualDistance: (workout.targetMetrics.distance || 0) * (scenario.performance === 'good' ? 1.0 : scenario.performance === 'poor' ? 0.7 : 0.85),
            completionRate: scenario.performance === 'good' ? 1.0 : scenario.performance === 'poor' ? 0.7 : 0.85,
            adherence: scenario.performance === 'poor' ? 'partial' : 'complete' as const,
            difficultyRating: scenario.performance === 'good' ? 5 : scenario.performance === 'poor' ? 8 : 6,
            paceDeviation: scenario.performance === 'good' ? 0 : scenario.performance === 'poor' ? 15 : 5
          },
          recoveryMetrics: {
            recoveryScore: scenario.recovery === 'high' ? 85 : scenario.recovery === 'low' ? 55 : 70,
            sleepQuality: scenario.recovery === 'high' ? 85 : scenario.recovery === 'low' ? 60 : 75,
            sleepDuration: scenario.recovery === 'high' ? 8 : scenario.recovery === 'low' ? 6 : 7,
            stressLevel: scenario.recovery === 'high' ? 20 : scenario.recovery === 'low' ? 60 : 40,
            muscleSoreness: scenario.recovery === 'high' ? 2 : scenario.recovery === 'low' ? 6 : 4,
            energyLevel: scenario.recovery === 'high' ? 8 : scenario.recovery === 'low' ? 4 : 6,
            motivation: scenario.recovery === 'high' ? 9 : scenario.recovery === 'low' ? 5 : 7
          }
        }));
        ...
        const analysis = adaptationEngine.analyzeProgress(completedWorkouts, plannedWorkouts);
        const latestProgress = cumulativeProgress[cumulativeProgress.length - 1];
        const modifications = adaptationEngine.suggestModifications(initialPlan, latestProgress, latestProgress.recoveryMetrics);
        ...
        const performanceMultiplier = scenario.performance === 'good' ? 1.01 : scenario.performance === 'poor' ? 0.99 : 1.0;
        currentFitnessState.vdot = currentFitnessState.vdot * performanceMultiplier;
        currentFitnessState.recoveryRate = scenario.recovery === 'high' ? 80 : scenario.recovery === 'low' ? 60 : 70;
      }
      ...
      expect(adaptedPlan.config.currentFitness?.vdot).toBeCloseTo(currentFitnessState.vdot, 2);
    });
  });
  ...
  describe('Performance Optimization', () => {
    it('should maintain performance targets for large plans', async () => {
      ...
      const runs = plan.workouts.filter(w => w.type === 'easy').map(w => ({
        date: w.date,
