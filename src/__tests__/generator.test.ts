...
describe('TrainingPlanGenerator', () => {
  ...
  describe('Volume Progression Patterns', () => {
    it('should implement progressive volume increase', () => {
      ...
      const firstQuarter = weeklyVolumes.slice(0, Math.floor(weeklyVolumes.length / 4));
      const lastQuarter = weeklyVolumes.slice(-Math.floor(weeklyVolumes.length / 4));
  ...
  describe('Short Plan Generation (4 weeks)', () => {
    ...
    it('should focus on race-specific work in short plans', () => {
      ...
      const raceSpecificWorkouts = plan.workouts.filter(w => 
        w.workout.segments?.some(s => s.pace?.includes('5K') || s.description?.includes('race'))
      );
  ...
  describe('Long Plan Generation (52 weeks)', () => {
    ...
    it('should implement periodization in long plans', () => {
      ...
      expect(phases.some(phase => phase.toLowerCase().includes('base'))).toBe(true);
    });
    ...
    it('should manage volume progression over long duration', () => {
      ...
      const avgFirst = firstMonth.reduce((a, b) => a + b, 0) / firstMonth.length;
      const avgMid = midPlan.reduce((a, b) => a + b, 0) / midPlan.length;
      const avgFinal = finalBuild.reduce((a, b) => a + b, 0) / finalBuild.length;
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle various goal types', () => {
      const goals = ['5k', '10k', 'half-marathon', 'marathon', 'ultra', 'general-fitness'] as const;
    ...
    it('should maintain data consistency throughout generation', () => {
      ...
      plan.workouts.forEach(workout => {
        expect(workout.date.getTime()).toBeGreaterThanOrEqual(planStartDate.getTime());
        expect(workout.date.getTime()).toBeLessThanOrEqual(expectedEndDate.getTime());
      });
