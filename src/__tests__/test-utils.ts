...
export const createMockFitnessAssessment = (
  overrides?: Partial<FitnessAssessment>,
): FitnessAssessment => ({
  vdot: 45,
  criticalSpeed: 12.5,
...
export const createMockTrainingPreferences = (
  overrides?: Partial<TrainingPreferences>,
): TrainingPreferences => ({
...
export const createMockEnvironmentalFactors = (
  overrides?: Partial<EnvironmentalFactors>,
): EnvironmentalFactors => ({
  altitude: 100,
...
export const createMockTrainingPlanConfig = (
...
export const createMockAdvancedPlanConfig = (
...
export const createMockTargetRace = (
  overrides?: Partial<TargetRace>,
): TargetRace => ({
...
export const createMockCompletedWorkout = (
  ...
  // Add backward compatibility fields for existing tests that expect them
  const extendedData = createExtendedCompletedWorkout(
    strictCompletedWorkout,
    {},
  );
...
export const createMockRecoveryMetrics = (
  overrides?: Partial<RecoveryMetrics>,
): RecoveryMetrics => {
      ...
      plannedWorkouts.push(
        createMockPlannedWorkout({
          ...
          targetMetrics: {
            ...
            intensity:
              type === "recovery" ? 50 : type === "threshold" ? 90 : 70,
 ...
 */
export const createMockPlannedWorkout = (
...
export const generateMockPlanWithAvailableDays = (
  ...
  const config = createMockAdvancedPlanConfig({
    preferences: {
      ...createMockTrainingPreferences(),
      availableDays,
      timeConstraints: availableDays.reduce(
        (constraints, day) => {
  ...
  for (let i = 0; i < Math.min(4, Math.ceil(weeks / 3)); i++) {
    ...
    blocks.push({
      ...
      startDate: startOfDay(
        addDays(testDateUtils.createTestDate("2024-01-01"), startWeek * 7),
      ),
      endDate: startOfDay(
        addDays(
          testDateUtils.createTestDate("2024-01-01"),
          (startWeek + blockWeeks) * 7,
        ),
      ),
      weeks: blockWeeks,
      focusAreas: ["aerobic", "threshold", "speed"][i % 3]
        ? [["aerobic", "threshold", "speed"][i % 3]]
        : ["aerobic"],
...
export const validateIntensityDistribution = (
  ...
  const easyCount = workouts.filter((w) =>
    ["recovery", "easy"].includes(w.type),
  ).length;
  const moderateCount = workouts.filter((w) =>
    ["steady", "tempo"].includes(w.type),
  ).length;
  const hardCount = workouts.filter((w) =>
    ["threshold", "vo2max", "speed"].includes(w.type),
  ).length;
  const veryHardCount = workouts.filter((w) =>
    ["hill_repeats", "fartlek"].includes(w.type),
  ).length;
...
export interface AdaptationMethodMapping {
  testMethod: string;
  actualMethod: string;
  parameterTransform?: <T extends unknown[], R extends unknown[]>(
    params: T,
  ) => R;
}
...
export const adaptationMethodMappings: AdaptationMethodMapping[] = [
  {
    ...
    parameterTransform: <T extends unknown[], R extends unknown[]>(
      ...
      return [completedWorkouts || [], plannedWorkouts || []] as unknown as R;
    },
  },
  {
    ...
    parameterTransform: <T extends unknown[], R extends unknown[]>(
      ...
      return [plan, progress, recovery] as unknown as R;
    },
  },
];
...
export const exportTestAdapter: ExportTestAdapter = {
  ...
  validateExportResult: (
    ...
    const expectedExtension =
      expectedFormat === "ical" ? "ics" : expectedFormat;
...
export const generatorTestDataBuilder: GeneratorTestDataBuilder = {
  ...
  validatePlanStructure: (plan: TrainingPlan) => {
    ...
    if (plan.workouts.length > 0) {
      const sortedWorkouts = [...plan.workouts].sort(
  ...
  alignDataStructures: (testData: any): AdvancedPlanConfig => {
    ...
    return {
      ...
      adaptationSettings:
        testData.adaptationSettings || baseConfig.adaptationSettings,
      // Ensure dates are properly formatted
      startDate: testData.startDate
        ? startOfDay(new Date(testData.startDate))
        : baseConfig.startDate,
      targetDate: testData.targetDate
        ? startOfDay(new Date(testData.targetDate))
        : baseConfig.targetDate,
    };
  },
};
...
export const testHelpers = {
  ...
  adaptMethod: (testMethod: string, implementation: any): Function => {
    const mapping = adaptationMethodMappings.find(
      (m) => m.testMethod === testMethod,
    );
    if (!mapping) {
    ...
    if (!actualMethod) {
      throw new Error(
        `Implementation does not have method: ${mapping.actualMethod}`,
      );
    }

    // Return a wrapped function that transforms parameters if needed
    return function <T extends unknown[]>(...args: T) {
      const transformedArgs = mapping.parameterTransform
        ? mapping.parameterTransform(args)
        : args;
   ...
   */
  createViaFactory: <T>(
    type: "philosophy" | "adaptation" | "exporter",
    ...args: unknown[]
  ): T => {
    switch (type) {
      case "philosophy":
        return PhilosophyFactory.create(
          args[0] as TrainingMethodology,
        ) as unknown as T;
      case "adaptation":
...
export const fitnessAssessmentMockGenerator: MockGenerator<FitnessAssessment> =
  {
    ...
    generateMany: (
      count: number,
      overrides?:
        | Partial<FitnessAssessment>
        | ((index: number) => Partial<FitnessAssessment>),
    ): FitnessAssessment[] => {
      return Array(count)
        .fill(null)
        .map((_, index) => {
    ...
    schema: {
      validate: (
        data: unknown,
      ): TypedResult<FitnessAssessment, TypeValidationError> => {
        ...
        if (
          !("vdot" in data) ||
          !("criticalSpeed" in data) ||
          !("lactateThreshold" in data)
        ) {
          return {
            success: false,
            error: new TypeValidationError(
...
export const trainingPlanConfigMockGenerator: MockGenerator<TrainingPlanConfig> =
  {
    ...
    generateMany: (
      count: number,
      overrides?:
        | Partial<TrainingPlanConfig>
        | ((index: number) => Partial<TrainingPlanConfig>),
    ): TrainingPlanConfig[] => {
      return Array(count)
        .fill(null)
        .map((_, index) => {
    ...
    schema: {
      ...
      validate: (
        data: unknown,
      ): TypedResult<TrainingPlanConfig, TypeValidationError> => {
        if (typeof data !== "object" || data === null) {
          return {
            success: false,
            error: new TypeValidationError(
...
export const advancedPlanConfigMockGenerator: MockGenerator<AdvancedPlanConfig> =
  {
    ...
    generateMany: (
      count: number,
      overrides?:
        | Partial<AdvancedPlanConfig>
        | ((index: number) => Partial<AdvancedPlanConfig>),
    ): AdvancedPlanConfig[] => {
      return Array(count)
        .fill(null)
        .map((_, index) => {
    ...
    validate: (instance: AdvancedPlanConfig): boolean => {
      return (
        trainingPlanConfigMockGenerator.validate(instance) &&
    ...
    schema: {
      ...
      validate: (
        data: unknown,
      ): TypedResult<AdvancedPlanConfig, TypeValidationError> => {
        const baseValidation =
          trainingPlanConfigMockGenerator.schema.validate(data);
        if (!baseValidation.success) {
          return baseValidation as TypedResult<
            AdvancedPlanConfig,
            TypeValidationError
          >;
        }
    ...
    metadata: {
      ...
      version: "1.0.0",
      description:
        "Generates mock advanced plan configuration with methodology support",
    },
  };
...
export const plannedWorkoutMockGenerator: MockGenerator<PlannedWorkout> = {
  ...
  generateMany: (
    ...
    overrides?:
      ...
      | ((index: number) => Partial<PlannedWorkout>),
  ): PlannedWorkout[] => {
    return Array(count)
      .fill(null)
      .map((_, index) => {
  ...
  schema: {
    ...
    validate: (
      data: unknown,
    ): TypedResult<PlannedWorkout, TypeValidationError> => {
      if (typeof data !== "object" || data === null) {
        return {
          success: false,
          error: new TypeValidationError(
...
export const completedWorkoutMockGenerator: MockGenerator<CompletedWorkout> = {
  ...
  generateMany: (
    count: number,
    overrides?:
      | Partial<CompletedWorkout>
      | ((index: number) => Partial<CompletedWorkout>),
  ): CompletedWorkout[] => {
    return Array(count)
      .fill(null)
      .map((_, index) => {
  ...
  validate: (instance: CompletedWorkout): boolean => {
    return (
      instance.plannedWorkout !== undefined &&
      instance.actualDuration > 0 &&
      instance.actualDistance > 0 &&
      instance.completionRate >= 0 &&
      instance.completionRate <= 1
    );
  },
  ...
  schema: {
    validate: (
      data: unknown,
    ): TypedResult<CompletedWorkout, TypeValidationError> => {
      if (typeof data !== "object" || data === null) {
        return {
          success: false,
          error: new TypeValidationError(
  ...
  metadata: {
    ...
    version: "1.0.0",
    description:
      "Generates mock completed workout data with planned workout relationship",
  },
};
...
export const trainingPreferencesMockGenerator: MockGenerator<TrainingPreferences> =
  {
    ...
    generateMany: (
      ...
      overrides?:
        ...
        | ((index: number) => Partial<TrainingPreferences>),
    ): TrainingPreferences[] => {
      return Array(count)
        .fill(null)
        .map((_, index) => {
    ...
    validate: (instance: TrainingPreferences): boolean => {
      return (
        instance.availableDays.length > 0 &&
    ...
    schema: {
      validate: (
        data: unknown,
      ): TypedResult<TrainingPreferences, TypeValidationError> => {
        if (typeof data !== "object" || data === null) {
          return {
            success: false,
            error: new TypeValidationError(
      ...
      properties: {
        availableDays: true,
        preferredIntensity: true,
        crossTraining: true,
        strengthTraining: true,
        timeConstraints: true,
      },
      required: ["availableDays", "timeConstraints"],
    ...
    metadata: {
      ...
      version: "1.0.0",
      description:
        "Generates mock training preferences with realistic time constraints",
    },
  };
...
export const targetRaceMockGenerator: MockGenerator<TargetRace> = {
  ...
  generateMany: (
    count: number,
    overrides?: Partial<TargetRace> | ((index: number) => Partial<TargetRace>),
  ): TargetRace[] => {
    return Array(count)
      .fill(null)
      .map((_, index) => {
  ...
  schema: {
    validate: (data: unknown): TypedResult<TargetRace, TypeValidationError> => {
      if (typeof data !== "object" || data === null) {
        return {
          success: false,
          error: new TypeValidationError("Expected object", "TargetRace", data),
        };
      }
    ...
    required: ["distance", "date", "priority"],
    name: "TargetRace",
  },
  ...
  metadata: {
    ...
    version: "1.0.0",
    description:
      "Generates mock target race data with realistic goal times and conditions",
  },
};
...
export const recoveryMetricsMockGenerator: MockGenerator<RecoveryMetrics> = {
  ...
  generateMany: (
    count: number,
    overrides?:
      | Partial<RecoveryMetrics>
      | ((index: number) => Partial<RecoveryMetrics>),
  ): RecoveryMetrics[] => {
    return Array(count)
      .fill(null)
      .map((_, index) => {
  ...
  validate: (instance: RecoveryMetrics): boolean => {
    return (
      instance.recoveryScore >= 0 &&
      instance.recoveryScore <= 100 &&
      instance.sleepQuality >= 0 &&
      instance.sleepQuality <= 100 &&
      instance.sleepDuration > 0 &&
      instance.stressLevel >= 0 &&
      instance.stressLevel <= 100
    );
  },
  ...
  schema: {
    validate: (
      data: unknown,
    ): TypedResult<RecoveryMetrics, TypeValidationError> => {
      if (typeof data !== "object" || data === null) {
        return {
          success: false,
          error: new TypeValidationError(
      ...
      if (
        ...
        !("sleepQuality" in data) ||
        !("sleepDuration" in data)
      ) {
        return {
          success: false,
          error: new TypeValidationError(
    ...
    required: ["recoveryScore", "sleepQuality", "sleepDuration", "stressLevel"],
  ...
  metadata: {
    ...
    version: "1.0.0",
    description:
      "Generates mock recovery metrics with proper validation ranges",
  },
};
...
export const runDataMockGenerator: MockGenerator<RunData> = {
  ...
  generateMany: (
    count: number,
    overrides?: Partial<RunData> | ((index: number) => Partial<RunData>),
  ): RunData[] => {
    return Array(count)
      .fill(null)
      .map((_, index) => {
  ...
  validate: (instance: RunData): boolean => {
    return (
      ...
      instance.distance > 0 &&
      instance.duration > 0 &&
      instance.avgPace !== undefined &&
      instance.avgPace > 0 &&
      instance.effortLevel !== undefined &&
      instance.effortLevel >= 1 &&
      instance.effortLevel <= 10
    );
  },
  ...
  schema: {
    validate: (data: unknown): TypedResult<RunData, TypeValidationError> => {
      if (typeof data !== "object" || data === null) {
        return {
          success: false,
          error: new TypeValidationError("Expected object", "RunData", data),
        };
      }
      if (
        !("date" in data) ||
  ...
  metadata: {
    ...
    version: "1.0.0",
    description:
      "Generates mock run data with realistic pacing and effort metrics",
  },
};

// Register common generators
defaultMockRegistry.register(
  "FitnessAssessment",
  fitnessAssessmentMockGenerator,
);
defaultMockRegistry.register(
  "TrainingPlanConfig",
  trainingPlanConfigMockGenerator,
);
defaultMockRegistry.register(
  "AdvancedPlanConfig",
  advancedPlanConfigMockGenerator,
);
...
defaultMockRegistry.register(
  "TrainingPreferences",
  trainingPreferencesMockGenerator,
);
...
export const createZonePerformanceDataSets = (): ZoneTestData[] => {
  ...
  return vdotValues.map((vdot) =>
    createMockZoneTestData({
      vdot,
...
export const createValidationPerformanceDataSets = (): ValidationTestCase[] => {
  ...
  return fieldCounts.map((count) => {
    ...
    const complexPreferences = {
      ...config.preferences,
      timeConstraints: Array.from({ length: count }, (_, i) => [
        i + 1,
        60,
...
export const calculatorTestDataMockGenerator: MockGenerator<CalculatorTestData> =
  {
    ...
    generateMany: (
      count: number,
      overrides?:
        | Partial<CalculatorTestData>
        | ((index: number) => Partial<CalculatorTestData>),
    ): CalculatorTestData[] => {
      return Array(count)
        .fill(null)
        .map((_, index) => {
    ...
    schema: {
      validate: (
        data: unknown,
      ): TypedResult<CalculatorTestData, TypeValidationError> => {
        if (typeof data !== "object" || data === null) {
          return {
            success: false,
            error: new TypeValidationError(
      ...
      properties: {
        ...
        expectedVDOT: true,
        tolerance: true,
        expectedCriticalSpeed: true,
        expectedLactateThreshold: true,
        context: true,
      },
      required: ["runs", "expectedVDOT"],
      name: "CalculatorTestData",
    },
    ...
    metadata: {
      ...
      version: "1.0.0",
      description:
        "Generates mock calculator test scenarios with physiological data",
    },
  };
...
export const validationTestCaseMockGenerator: MockGenerator<ValidationTestCase> =
  {
    ...
    generateMany: (
      count: number,
      overrides?:
        | Partial<ValidationTestCase>
        | ((index: number) => Partial<ValidationTestCase>),
    ): ValidationTestCase[] => {
      return Array(count)
        .fill(null)
        .map((_, index) => {
    ...
    schema: {
      validate: (
        data: unknown,
      ): TypedResult<ValidationTestCase, TypeValidationError> => {
        if (typeof data !== "object" || data === null) {
          return {
            success: false,
            error: new TypeValidationError(
    ...
    metadata: {
      ...
      version: "1.0.0",
      description:
        "Generates mock validation test cases with error expectations",
    },
  };
...
export const workoutTestScenarioMockGenerator: MockGenerator<WorkoutTestScenario> =
  {
    ...
    generateMany: (
      count: number,
      overrides?:
        | Partial<WorkoutTestScenario>
        | ((index: number) => Partial<WorkoutTestScenario>),
    ): WorkoutTestScenario[] => {
      return Array(count)
        .fill(null)
        .map((_, index) => {
    ...
    validate: (instance: WorkoutTestScenario): boolean => {
      return (
        instance.type.length > 0 &&
        instance.duration > 0 &&
        instance.intensity >= 0 &&
        instance.intensity <= 100 &&
    ...
    schema: {
      validate: (
        data: unknown,
      ): TypedResult<WorkoutTestScenario, TypeValidationError> => {
        if (typeof data !== "object" || data === null) {
          return {
            success: false,
            error: new TypeValidationError(
    ...
    metadata: {
      ...
      version: "1.0.0",
      description:
        "Generates mock workout test scenarios with TSS and recovery expectations",
    },
  };
...
export const zoneTestDataMockGenerator: MockGenerator<ZoneTestData> = {
  ...
  generateMany: (
    count: number,
    overrides?:
      | Partial<ZoneTestData>
      | ((index: number) => Partial<ZoneTestData>),
  ): ZoneTestData[] => {
    return Array(count)
      .fill(null)
      .map((_, index) => {
  ...
  schema: {
    validate: (
      data: unknown,
    ): TypedResult<ZoneTestData, TypeValidationError> => {
      if (typeof data !== "object" || data === null) {
        return {
          success: false,
          error: new TypeValidationError(
      ...
      if (
        !("maxHR" in data) ||
        !("thresholdPace" in data) ||
        !("expectedZones" in data)
      ) {
        return {
          success: false,
          error: new TypeValidationError(
  ...
  metadata: {
    ...
    version: "1.0.0",
    description:
      "Generates mock zone test data with expected training zone boundaries",
  },
};
...
defaultMockRegistry.register(
  "CalculatorTestData",
  calculatorTestDataMockGenerator,
);
defaultMockRegistry.register(
  "ValidationTestCase",
  validationTestCaseMockGenerator,
);
defaultMockRegistry.register(
  "WorkoutTestScenario",
  workoutTestScenarioMockGenerator,
);
...
export class EdgeCaseDataBuilder {
   ...
   */
  static createMinimumViablePlan(
    overrides?: Partial<TrainingPlanConfig>,
  ): TrainingPlanConfig {
    return createMockTrainingPlanConfig({
      ...
      currentFitness: createMockFitnessAssessment({
        ...
        longestRecentRun: 5, // 5km longest run for beginner
      }),
      ...overrides,
    });
  }
   ...
   */
  static createMaximumComplexityPlan(
    overrides?: Partial<AdvancedPlanConfig>,
  ): AdvancedPlanConfig {
    ...
    for (let i = 0; i < 12; i++) {
      races.push(
        createMockTargetRace({
          distance:
            i % 4 === 0 ? "marathon" : i % 3 === 0 ? "half-marathon" : "10k",
    ...
    return createMockAdvancedPlanConfig({
      ...
      currentFitness: createMockFitnessAssessment({
        ...
        longestRecentRun: 35, // Very long runs
      }),
      ...overrides,
    });
  }
  ...
  static createBoundaryValueData(): {
    ...
    const boundaryConfigs = [
      ...
      createMockTrainingPlanConfig({
        currentFitness: createMockFitnessAssessment({
          ...
          longestRecentRun: 50, // Ultra distance
        }),
      }),
    ];
    ...
    const boundaryAssessments = [
      ...
      createMockFitnessAssessment({
        ...
        longestRecentRun: 100, // Extreme distance
      }),
    ];
...
export class InvalidDataBuilder {
   ...
   */
  static createInvalidRunData(): Array<{
    runData: any;
   ...
   */
  static createCalculationEdgeCases(): Array<{
    input: any;
    operation: string;
  }> {
    return [
    ...
    return [
      ...
      },
      {
        ...
        data: {
          vdotRange: Array(100)
            .fill(null)
            .map((_, i) => 20 + i * 0.5), // VDOT 20-70
          heartRateRange: Array(100)
            .fill(null)
            .map((_, i) => 120 + i * 2), // HR 120-320
        },
    ...
    return [
      ...
      },
      {
        ...
        data: {
          plans: Array(10)
            .fill(null)
            .map(() => EdgeCaseDataBuilder.createMaximumComplexityPlan()),
        },
