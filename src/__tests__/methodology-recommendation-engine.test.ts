...
describe('MethodologyRecommendationEngine', () => {
  ...
  describe('Requirement 5.2: User Profile-Based Recommendations', () => {
    it('should recommend methodologies based on user profile', () => {
      ...
      expect(result.primaryRecommendation.methodology).toMatch(/daniels|lydiard|pfitzinger/);
      expect(result.primaryRecommendation.compatibilityScore).toBeGreaterThan(70);
    ...
    it('should recommend Daniels for scientific approach runners', () => {
      ...
      expect(result.primaryRecommendation.compatibilityScore).toBeGreaterThan(85);
    ...
    it('should recommend appropriate methodology for Boston qualifiers', () => {
      ...
      expect(result.primaryRecommendation.methodology).toMatch(/pfitzinger|daniels/);
      expect(result.primaryRecommendation.compatibilityScore).toBeGreaterThan(85);
      
      // Check that the recommendation includes marathon improvement
      const hasMarathonImprovement = result.primaryRecommendation.expectedOutcomes.some(outcome =>
        outcome.match(/marathon.*improvement|improve.*marathon|VDOT improvement/i)
      );
  ...
  describe('Requirement 5.4: Compatibility Scoring System', () => {
      ...
      const profiles: UserProfile[] = [
        {
          ...
          experience: 'advanced',
          currentFitness: { vdot: 55, weeklyMileage: 45, longestRecentRun: 18, trainingAge: 8 },
          trainingPreferences: {
            availableDays: [0, 1, 2, 3, 4, 5, 6],
            preferredIntensity: 'high',
            crossTraining: false,
        ...
        },
        {
          ...
          experience: 'beginner',
          currentFitness: { vdot: 40, weeklyMileage: 15, longestRecentRun: 6, trainingAge: 0.5 },
          trainingPreferences: {
            availableDays: [0, 1, 2, 3, 4, 5, 6],
            preferredIntensity: 'low',
            crossTraining: false,
          ...
          timeAvailability: 12,
          preferredApproach: 'intuitive',
          injuryHistory: ['Previous injury 1', 'Previous injury 2', 'Previous injury 3']
        }
      ];

      const results = profiles.map(profile => engine.recommendMethodology(profile));

      // First profile should strongly recommend Daniels
      expect(results[0].primaryRecommendation.methodology).toBe('daniels');
      expect(results[0].primaryRecommendation.compatibilityScore).toBeGreaterThan(90);

      // Second profile should strongly recommend Lydiard
      expect(results[1].primaryRecommendation.methodology).toBe('lydiard');
      expect(results[1].primaryRecommendation.compatibilityScore).toBeGreaterThan(85);

      // All recommendations should have scores between 0-100
      results.forEach(result => {
        expect(result.primaryRecommendation.compatibilityScore).toBeGreaterThanOrEqual(0);
        expect(result.primaryRecommendation.compatibilityScore).toBeLessThanOrEqual(100);
    ...
    it('should provide detailed scoring breakdown', () => {
      ...
      expect(result.rationale.scoringBreakdown).toHaveProperty('experienceMatch');
      expect(result.rationale.scoringBreakdown).toHaveProperty('goalAlignment');
      expect(result.rationale.scoringBreakdown).toHaveProperty('timeAvailability');
      expect(result.rationale.scoringBreakdown).toHaveProperty('injuryHistory');
      expect(result.rationale.scoringBreakdown).toHaveProperty('trainingApproach');
      expect(result.rationale.scoringBreakdown).toHaveProperty('environmentalFactors');
      expect(result.rationale.scoringBreakdown).toHaveProperty('strengthsWeaknesses');
  ...
  describe('Requirement 5.8: Recommendation Rationale Generation', () => {
    it('should generate comprehensive rationale', () => {
      ...
      result.rationale.primaryFactors.forEach(factor => {
        expect(factor).toMatch(/match|alignment|fit|compatibility|suitability|targeting/i);
      });
    });
    ...
    it('should provide methodology-specific advantages', () => {
      const profiles: UserProfile[] = [
        ...
        },
        {
          // Lydiard candidate
          experience: 'novice',
          currentFitness: { vdot: 42, weeklyMileage: 20, longestRecentRun: 8, trainingAge: 1.5 },
          trainingPreferences: {
            availableDays: [0, 1, 2, 3, 4, 5, 6],
            preferredIntensity: 'low',
            crossTraining: false,
  ...
  describe('Warnings and Considerations', () => {
    it('should generate warnings for inappropriate recommendations', () => {
      const beginner: UserProfile = {
        experience: 'beginner',
        currentFitness: { vdot: 38, weeklyMileage: 10, longestRecentRun: 4, trainingAge: 0.3 },
        trainingPreferences: {
          availableDays: [0, 1, 2, 3, 4, 5, 6],
          preferredIntensity: 'low',
          crossTraining: false,
      ...
      if (result.primaryRecommendation.methodology === 'pfitzinger') {
        ...
        expect(result.warnings![0]).toMatch(/requires significant running base/i);
      }
    });
    ...
    it('should identify considerations for each methodology', () => {
      const userProfile: UserProfile = {
        ...
        preferredApproach: 'structured',
        injuryHistory: ['IT band', 'Shin splints', 'Achilles', 'Plantar fasciitis'] // Many injuries
      };

      const result = engine.recommendMethodology(userProfile);

      expect(result.primaryRecommendation.considerations).toBeDefined();
      expect(result.primaryRecommendation.considerations.length).toBeGreaterThan(0);
  ...
  describe('Expected Outcomes', () => {
    it('should predict realistic outcomes', () => {
      ...
      expect(result.primaryRecommendation.expectedOutcomes.length).toBeGreaterThan(0);

      // Outcomes should be specific and measurable
      result.primaryRecommendation.expectedOutcomes.forEach(outcome => {
        expect(outcome).toMatch(/improvement|development|ability|fitness|toughness/i);
      });
    });
    ...
    it('should estimate adaptation time', () => {
      const profiles: UserProfile[] = [
        {
          ...
          experience: 'advanced',
          currentFitness: { vdot: 55, weeklyMileage: 50, longestRecentRun: 20, trainingAge: 8 },
          trainingPreferences: {
            availableDays: [0, 1, 2, 3, 4, 5, 6],
            preferredIntensity: 'high',
            crossTraining: false,
      ...
      expect(results[0].primaryRecommendation.timeToAdapt).toBeLessThanOrEqual(6);

      // Beginner should need more time
      expect(results[1].primaryRecommendation.timeToAdapt).toBeGreaterThanOrEqual(6);
    });
  });
  ...
  describe('Recommendation Quiz', () => {
    ...
    it('should score quiz answers correctly', () => {
      ...
      const answers = [
        ...
        { questionId: 'motivations', answer: ['improve_times', 'qualify_boston'] },
      ...
      expect(userProfile.strengthsAndWeaknesses!.strengths).toContain('consistency');
    });
    ...
    it('should handle quiz answers to generate recommendations', () => {
      ...
      const beginnerAnswers = [
        ...
        { questionId: 'motivations', answer: ['finish_first_race', 'stay_healthy'] },
      ...
      expect(recommendation.primaryRecommendation.methodology).toMatch(/daniels|lydiard|pfitzinger/);
      expect(recommendation.primaryRecommendation.compatibilityScore).toBeGreaterThan(60);
    });
  });
