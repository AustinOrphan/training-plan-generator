...
describe('MethodologyCustomizationEngine', () => {
    ...
    it('should create methodology-specific configurations', () => {
      ...
      methodologies.forEach(methodology => {
        ...
        if (methodology === 'lydiard') {
          expect(config.baseConfig.periodizationModel.phaseTransitions).toBe('sharp');
          expect(config.baseConfig.recoveryProtocol.completeRestDays).toBeGreaterThan(0);
        } else {
          expect(config.baseConfig.periodizationModel.phaseTransitions).toBe('gradual');
        }
      });
    });
  ...
  describe('Requirement 6.2: Individual Adaptation Pattern Tracking', () => {
    ...
    it('should update configuration based on learned patterns', () => {
      const config = engine.initializeConfiguration('user-1', 'daniels', mockUserProfile);
    ...
    it('should optimize performance based on individual response', () => {
      ...
      const thresholdOpt = optimizations.find(opt => opt.targetMetric === 'threshold');
      ...
      const enduranceOpt = optimizations.find(opt => opt.targetMetric === 'endurance');
  ...
  describe('Requirement 6.3: Environmental Factor Adaptations', () => {
    it('should apply altitude adjustments', () => {
      ...
      expect(altitudeMod!.suggestedChanges.intensityReduction).toBeGreaterThan(0);
    });
    ...
    it('should apply terrain-specific adaptations', () => {
      ...
      expect(terrainMod!.suggestedChanges.substituteWorkoutType).toBe('hill_repeats');
    });
  });
  ...
  describe('Requirement 6.4: Safe Customization Options', () => {
    it('should resolve methodology conflicts safely', () => {
      ...
      const conflicts = [{
        ...
        description: 'User needs more recovery than Daniels typically prescribes',
        severity: 'high' as const
      }];

      const resolutions = engine.resolveMethodologyConflicts('user-1', conflicts);
    ...
    it('should maintain safe customization constraints', () => {
      ...
      expect(config.baseConfig.volumeProgression.weeklyIncrease).toBeLessThan(10);
    });
  });
  ...
  describe('Requirement 6.5: Experience-based Advanced Features', () => {
    it('should unlock advanced features for experienced runners', () => {
      ...
      const doubleThreshold = unlockedFeatures.find(f => f.id === 'double_threshold');
    ...
    it('should unlock time-based features', () => {
      ...
      const raceSimulation = unlockedFeatures.find(f => f.id === 'race_simulation');
    ...
    it('should not unlock advanced features for beginners', () => {
      ...
      const advancedFeatures = unlockedFeatures.filter(f => 
        f.id === 'double_threshold' || f.id === 'super_compensation'
      );
  ...
  describe('Requirement 6.6: Injury Prevention Modifications', () => {
    ...
    it('should mitigate current risk factors', () => {
      ...
      expect(riskMod.suggestedChanges.additionalRecoveryDays).toBeGreaterThan(0);
    });
  });
  ...
  describe('Requirement 6.7: Performance Plateau Breakthrough', () => {
    it('should suggest methodology-specific breakthrough strategies', () => {
      ...
      const vdotBreakthrough = strategies.find(s => s.id === 'vdot_breakthrough');
    ...
    it('should provide different strategies for different methodologies', () => {
      ...
      const aerobicBreakthrough = lydiardStrategies.find(s => s.id === 'aerobic_breakthrough');
      ...
      const thresholdBreakthrough = pfitzingerStrategies.find(s => s.id === 'threshold_breakthrough');
    ...
    it('should sort strategies by success probability', () => {
      ...
      for (let i = 1; i < strategies.length; i++) {
        expect(strategies[i - 1].successProbability)
          .toBeGreaterThanOrEqual(strategies[i].successProbability);
      }
    });
  });
  ...
  describe('Customization Analysis', () => {
    it('should analyze current customization state', () => {
      ...
      expect(analysis.currentState.adherenceToPhilosophy).toBeGreaterThanOrEqual(0);
      expect(analysis.currentState.adherenceToPhilosophy).toBeLessThanOrEqual(100);
      expect(analysis.currentState.customizationLevel).toMatch(/minimal|moderate|extensive/);
      expect(analysis.currentState.effectivenessScore).toBeGreaterThanOrEqual(0);
    ...
    it('should provide customization recommendations', () => {
      ...
      if (analysis.recommendations.length > 0) {
        ...
        expect(recommendation.category).toMatch(/performance|injury_prevention|plateau_breaking|environmental/);
    ...
    it('should identify customization warnings', () => {
      ...
      const lowEffectivenessWorkouts = createMockCompletedWorkouts(20).map(w => ({
        ...w,
  ...
  describe('Configuration Persistence', () => {
    ...
    it('should update configuration last updated timestamp', () => {
      ...
      setTimeout(() => {
        ...
        const updatedConfig = engine.getConfiguration('user-1');
        expect(updatedConfig!.lastUpdated.getTime()).toBeGreaterThan(initialTimestamp.getTime());
      }, 10);
    });
  });
});