...
describe("Multi-Methodology End-to-End Tests", () => {
  ...
  describe("Scenario 1: Complete Daniels Plan", () => {
    it("should generate 16-week marathon plan using Daniels methodology with accurate VDOT and 80/20 distribution", async () => {
      ...
      const danielsConfig: AdvancedPlanConfig = {
        ...
        description:
          "Complete 16-week marathon plan using Jack Daniels methodology",
      };
      ...
      const intervalWorkouts = plan.workouts.filter(
        (w) => w.type === "intervals",
      );
  ...
  describe("Scenario 2: Lydiard Base Building", () => {
    it("should create 12-week base phase plan with aerobic emphasis and hill training", async () => {
      ...
      const hillWorkouts = plan.workouts.filter(
        (w) =>
          w.type === "hill_repeats" ||
      ...
      const longRuns = plan.workouts.filter(
        (w) => w.type === "long" || w.type === "long_run",
      );
      ...
      const timeBasedWorkouts = plan.workouts.filter(
        (w) => w.targetMetrics?.duration && !w.targetMetrics?.pace,
      );
  ...
  describe("Scenario 3: Pfitzinger Marathon", () => {
    it("should generate race-specific plan with medium-long runs and threshold progression", async () => {
      ...
      const mediumLongRuns = plan.workouts.filter((w) => {
        ...
        return (
          (w.type === "medium_long" ||
            ...
            w.type === "long_run") &&
          distance &&
          distance >= 8 // Lower threshold to catch more runs
        );
      });
      ...
      if (weeklyThresholdVolumes.length > 4) {
        ...
        const firstHalf = weeklyThresholdVolumes.slice(
          0,
          Math.floor(weeklyThresholdVolumes.length / 2),
        );
        const secondHalf = weeklyThresholdVolumes.slice(
          Math.floor(weeklyThresholdVolumes.length / 2),
        );
        const avgFirstHalf =
      ...
      const raceSpecificWorkouts = plan.workouts.filter(
        (w) =>
      ...
      const hardDays = Array.from(workoutsByDayOfWeek.entries())
        .filter(([_, types]) =>
          types.some((t) =>
            ["tempo", "threshold", "intervals", "vo2max"].includes(t),
          ),
        )
        .map(([day]) => day);
  ...
  describe("Scenario 4: Philosophy Comparison", () => {
    it("should compare all three methodologies for the same goal race and show meaningful differences", async () => {
      ...
      const intensityDistributions = new Map<
        TrainingMethodology,
      ...
      for (const [methodology, plan] of plans) {
        ...
        const hardWorkouts = plan.workouts.filter((w) =>
          ["tempo", "threshold", "intervals", "vo2max", "speed"].includes(
            w.workout.type,
          ),
        );
      ...
      const workoutTypeCounts = new Map<
        TrainingMethodology,
        Map<string, number>
      >();
      ...
      for (const [methodology, plan] of plans) {
        ...
        plan.workouts.forEach((w) => {
          typeCounts.set(
            w.workout.type,
            (typeCounts.get(w.workout.type) || 0) + 1,
          );
        });
      ...
      const danielsIntervals =
        workoutTypeCounts.get("daniels")?.get("intervals") || 0;
      const lydiardIntervals =
        workoutTypeCounts.get("lydiard")?.get("intervals") || 0;
      ...
      expect(pfitzingerThreshold).toBeGreaterThanOrEqual(
        danielsThreshold * 0.8,
      );

      // Verify phase emphasis differences
      const phaseDistributions = new Map<
        TrainingMethodology,
        Map<string, number>
      >();
  ...
  describe("Scenario 5: Methodology Switching", () => {
    it("should handle switching from one methodology to another mid-plan with proper transition", async () => {
      ...
      const completedWorkouts = initialPlan.workouts.filter(
        (w) => w.date && new Date(w.date) < transitionDate,
      ).length;
      ...
      const firstWeekWorkouts = newPlan.workouts.filter((w) => {
        const weekDiff = differenceInWeeks(w.scheduledDate, transitionDate);
        return weekDiff === 0;
      });

      const secondWeekWorkouts = newPlan.workouts.filter((w) => {
        const weekDiff = differenceInWeeks(w.scheduledDate, transitionDate);
        return weekDiff === 1;
      });

      // Check intensity progression during transition
      const firstWeekHardCount = firstWeekWorkouts.filter((w) =>
        ["tempo", "threshold", "intervals"].includes(w.workout.type),
      ).length;

      const secondWeekHardCount = secondWeekWorkouts.filter((w) =>
        ["tempo", "threshold", "intervals"].includes(w.workout.type),
      ).length;
      ...
      const postTransitionWorkouts = newPlan.workouts.filter((w) => {
        const weekDiff = differenceInWeeks(w.scheduledDate, transitionDate);
        return weekDiff >= 3; // After transition period
      });

      const postTransitionEasy = postTransitionWorkouts.filter((w) =>
        ["easy", "recovery", "long"].includes(w.workout.type),
      ).length;

      const postTransitionHard = postTransitionWorkouts.filter((w) =>
        ["tempo", "threshold", "intervals", "vo2max"].includes(w.workout.type),
      ).length;
