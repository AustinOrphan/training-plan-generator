...
import { calculateTrainingPaces, calculateVDOTFromRace, DanielsTrainingPaces } from '../zones';
import { TrainingPlan, TrainingPhase, TrainingBlock, PlannedWorkout, Workout, WorkoutType } from '../types';
import { 
...
describe('Daniels Methodology Validation and Testing', () => {
  ...
  describe('VDOT Calculations', () => {
    describe('calculateTrainingPaces', () => {
      ...
      it('should handle different VDOT values correctly', () => {
        ...
        expect(highVDOT.threshold.target).toBeLessThan(lowVDOT.threshold.target);
  ...
  describe('Workout Selection Accuracy', () => {
    describe('Phase-Specific Selection Logic', () => {
      ...
      it('should progressively introduce quality work in base phase', () => {
        ...
        const week2Threshold = danielsPhilosophy.selectWorkout('threshold', 'base', 2);
        expect(['LACTATE_THRESHOLD_2X20', 'THRESHOLD_PROGRESSION'].includes(week2Threshold)).toBe(true);

        // Week 5+: Threshold allowed (but may be specific threshold workout)
        const week5Threshold = danielsPhilosophy.selectWorkout('threshold', 'base', 5);
        expect(['LACTATE_THRESHOLD_2X20', 'THRESHOLD_PROGRESSION'].includes(week5Threshold)).toBe(true);
      });

      it('should avoid VO2max work in base phase', () => {
        const week1VO2Max = danielsPhilosophy.selectWorkout('vo2max', 'base', 1);
        const week5VO2Max = danielsPhilosophy.selectWorkout('vo2max', 'base', 5);
        ...
        expect(['TEMPO_CONTINUOUS', 'VO2MAX_4X4', 'VO2MAX_5X3'].includes(week1VO2Max)).toBe(true);
        expect(['TEMPO_CONTINUOUS', 'VO2MAX_4X4', 'VO2MAX_5X3'].includes(week5VO2Max)).toBe(true);
      });

      it('should emphasize threshold and intervals in build phase', () => {
        const buildThreshold = danielsPhilosophy.selectWorkout('threshold', 'build', 2);
        const buildVO2Max = danielsPhilosophy.selectWorkout('vo2max', 'build', 3);
      ...
      it('should reduce volume but maintain intensity in taper phase', () => {
        const taperThreshold = danielsPhilosophy.selectWorkout('threshold', 'taper', 1);
        const taperVO2Max = danielsPhilosophy.selectWorkout('vo2max', 'taper', 1);
      ...
      it('should prioritize recovery in recovery phase', () => {
        const recoveryEasy = danielsPhilosophy.selectWorkout('easy', 'recovery', 1);
        const recoveryThreshold = danielsPhilosophy.selectWorkout('threshold', 'recovery', 1);
    ...
    describe('Workout Customization with VDOT', () => {
      it('should apply VDOT-based paces to workout segments', () => {
        ...
        const customized = danielsPhilosophy.customizeWorkout(template, 'build', 4, vdot);
        ...
        customized.segments.forEach(segment => {
          ...
          expect(segment.paceTarget.max).toBeGreaterThan(segment.paceTarget.min);
        });
      });
      ...
      it('should map workout types to appropriate pace zones', () => {
        ...
        testCases.forEach(({ type, expectedZone }) => {
          ...
          const expectedPace = paces[expectedZone as keyof DanielsTrainingPaces];
      ...
      it('should apply phase-specific intensity adjustments', () => {
        ...
        const baseCustomization = danielsPhilosophy.customizeWorkout(template, 'base', 2, vdot);
        const buildCustomization = danielsPhilosophy.customizeWorkout(template, 'build', 2, vdot);
        const peakCustomization = danielsPhilosophy.customizeWorkout(template, 'peak', 2, vdot);

        // Base phase should have reduced intensity
        expect(baseCustomization.segments[0].intensity).toBeLessThan(buildCustomization.segments[0].intensity);
        
        // Peak phase should have slightly higher intensity
        expect(peakCustomization.segments[0].intensity).toBeGreaterThanOrEqual(buildCustomization.segments[0].intensity);
      });

      it('should enhance segment descriptions with pace information', () => {
        const template = createMockWorkoutTemplate('threshold');
        const customized = danielsPhilosophy.customizeWorkout(template, 'build', 4, 50);
  ...
  describe('Intensity Distribution Validation', () => {
    it('should maintain 80/20 distribution in enhanced plans', () => {
      const plan = {
        ...
        workouts: [
          ...
          createMockPlannedWorkout({ type: 'easy', targetMetrics: { duration: 60, intensity: 70, tss: 40, load: 40, distance: 8 } }),
          createMockPlannedWorkout({ type: 'threshold', targetMetrics: { duration: 30, intensity: 88, tss: 50, load: 50, distance: 5 } }),
        ]
      };
    ...
    it('should detect and report intensity distribution violations', () => {
      const violatingPlan = {
        ...mockPlan,
        workouts: [
          createMockPlannedWorkout({ type: 'threshold', targetMetrics: { duration: 40, intensity: 88, tss: 60, load: 60, distance: 6 } }),
          createMockPlannedWorkout({ type: 'vo2max', targetMetrics: { duration: 30, intensity: 98, tss: 50, load: 50, distance: 5 } }),
          createMockPlannedWorkout({ type: 'speed', targetMetrics: { duration: 25, intensity: 105, tss: 45, load: 45, distance: 4 } }),
    ...
    it('should provide actionable recommendations for distribution improvements', () => {
      const nonCompliantPlan = {
        ...
        workouts: [
          ...
          createMockPlannedWorkout({ type: 'threshold', targetMetrics: { duration: 30, intensity: 88, tss: 50, load: 50, distance: 5 } }),
          createMockPlannedWorkout({ type: 'vo2max', targetMetrics: { duration: 25, intensity: 98, tss: 45, load: 45, distance: 4 } }),
        ]
      };

      const report = danielsPhilosophy.generateIntensityReport(nonCompliantPlan);
  ...
  describe('Performance and Optimization', () => {
    ...
    it('should handle large plans efficiently', () => {
      const largePlan = {
        ...mockPlan,
        workouts: Array(100).fill(null).map((_, i) => 
          createMockPlannedWorkout({ 
            type: i % 2 === 0 ? 'easy' : 'threshold',
            targetMetrics: { duration: 45, intensity: 75, tss: 40, load: 40, distance: 6 }
          })
        )
      };
