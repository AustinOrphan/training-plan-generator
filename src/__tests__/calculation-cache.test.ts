...
describe('Calculation Cache and Optimization', () => {
  ...
  describe('Training Paces Caching', () => {
    it('should cache training paces by methodology', () => {
      ...
      const paces1 = calculateTrainingPacesCached(vdot, 'daniels', mockCalculator);
      const paces2 = calculateTrainingPacesCached(vdot, 'daniels', mockCalculator);
    ...
    it('should cache separately for different methodologies', () => {
      ...
      const danielsPaces = calculateTrainingPacesCached(vdot, 'daniels', danielsCalculator);
      const lydiardPaces = calculateTrainingPacesCached(vdot, 'lydiard', lydiardCalculator);
  ...
  describe('Memory Monitoring', () => {
    it('should track memory snapshots', () => {
      ...
      const memoryIncrease = MemoryMonitor.getMemoryIncrease('start', 'after-allocation');
  ...
  describe('Optimization Analysis', () => {
    ...
    it('should identify optimization opportunities', () => {
      ...
      expect(analysis.recommendations.some(r => 
        r.includes('Consider optimizing')
      )).toBe(true);
    });
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle very large datasets', () => {
      ...
      expect(() => calculateFitnessMetricsCached(largeRunHistory)).not.toThrow();
    ...
    it('should maintain cache integrity under concurrent access', async () => {
      ...
      const promises = Array(10).fill(null).map(() => 
        Promise.resolve(calculateVDOTCached(runHistory))
      );
