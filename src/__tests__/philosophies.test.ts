...
describe("PhilosophyFactory", () => {
  ...
  it("should throw error for unknown philosophy", () => {
    ...
    expect(() => PhilosophyFactory.create(invalidMethodology)).toThrow(
      "Unknown training methodology: unknown",
    );
  });
});
...
describe("DanielsPhilosophy", () => {
  ...
  it("should get phase-specific intensity distribution", () => {
    ...
    expect(
      baseDistribution.easy + baseDistribution.moderate + baseDistribution.hard,
    ).toBe(100);
  });
...
describe("Philosophy Integration", () => {
  ...
  it("should validate different intensity distributions", () => {
    ...
    validateIntensityDistribution(
      mockWorkouts.daniels,
      daniels.intensityDistribution,
    );
    validateIntensityDistribution(
      mockWorkouts.lydiard,
      lydiard.intensityDistribution,
    );
    validateIntensityDistribution(
      mockWorkouts.pfitzinger,
      pfitzinger.intensityDistribution,
    );
  });
});
