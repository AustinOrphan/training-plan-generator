...
describe('CustomWorkoutGenerator', () => {
  describe('Daniels Methodology Generator', () => {
    ...
    it('should generate VDOT-based tempo workout', () => {
      ...
      const tempoSegment = result.workout.segments.find(seg => 
        seg.intensity === 88 && seg.zone.name === 'Tempo'
      );
    ...
    it('should generate VO2max intervals with proper recovery', () => {
      ...
      const intervals = result.workout.segments.filter(seg => seg.intensity === 95);
      expect(intervals.length).toBeGreaterThan(0);
      
      // Check for recovery segments
      const recoveries = result.workout.segments.filter(seg => seg.intensity === 60);
    ...
    it('should generate speed repetitions with full recovery', () => {
      ...
      const reps = result.workout.segments.filter(seg => seg.intensity === 98);
      expect(reps.length).toBeGreaterThan(0);
      
      // Check recovery is longer than work
      const firstRep = reps[0];
      const recovery = result.workout.segments.find(seg => 
        seg.intensity === 50 && seg.description.includes('recovery')
      );
  ...
  describe('Lydiard Methodology Generator', () => {
    ...
    it('should generate aerobic-emphasis long run', () => {
      ...
      const aerobicSegments = result.workout.segments.filter(seg => seg.intensity <= 65);
      const totalDuration = result.workout.segments.reduce((sum, seg) => sum + seg.duration, 0);
      expect(aerobicSegments.reduce((sum, seg) => sum + seg.duration, 0)).toBe(totalDuration);
    });
  ...
  describe('Pfitzinger Methodology Generator', () => {
    ...
    it('should generate LT intervals for longer threshold sessions', () => {
      ...
      const ltIntervals = result.workout.segments.filter(seg => 
      ...
      const recoveries = result.workout.segments.filter(seg => 
        seg.intensity === 65 && seg.description.includes('Recovery')
      );
    ...
    it('should generate medium-long run with quality segment', () => {
      ...
      const qualitySegment = result.workout.segments.find(seg => 
        seg.intensity === 82 && seg.description.includes('Marathon pace')
      );
      expect(qualitySegment).toBeDefined();
      
      // Should still have easy portions
      const easySegments = result.workout.segments.filter(seg => seg.intensity === 70);
  ...
  describe('Environmental and Constraint Handling', () => {
    ...
    it('should adjust for hot weather', () => {
      ...
      const tempoSegment = result.workout.segments.find(seg => 
        seg.zone.name === 'Tempo' && seg.description.includes('Tempo')
      );
  ...
  describe('Constraint Validation and Warnings', () => {
    ...
    it('should warn about high altitude impacts', () => {
      ...
      expect(result.constraints).toContain('High altitude may significantly impact performance');
    });
  ...
  describe('Alternative Workout Generation', () => {
    ...
    it('should generate alternatives when compliance is low', () => {
      ...
      result.alternatives?.forEach(alt => {
        const avgIntensity = alt.segments.reduce(
          (sum, seg) => sum + seg.intensity * seg.duration, 0
  ...
  describe('Methodology-Specific Rules', () => {
    it('should enforce Daniels pacing zones', () => {
      ...
      const workoutTypes: Array<{ type: WorkoutType; expectedIntensity: number }> = [
      ...
      workoutTypes.forEach(({ type, expectedIntensity }) => {
        ...
        const hasCorrectIntensity = result.workout.segments.some(seg => 
          seg.intensity === expectedIntensity
        );
    ...
    it('should enforce Lydiard aerobic emphasis', () => {
      ...
      const totalTime = result.workout.segments.reduce((sum, seg) => sum + seg.duration, 0);
    ...
    it('should implement Pfitzinger LT focus', () => {
      ...
      phases.forEach(phase => {
        ...
        const ltSegments = result.workout.segments.filter(seg => 
          seg.intensity >= 84 && seg.intensity <= 88
        );
