  ...
  describe('createCustomWorkout Function', () => {
    ...
    describe('Invalid Workout Type Handling', () => {
      it('should handle invalid workout types gracefully', () => {
        ...
        const invalidWorkout = createCustomWorkout('invalid' as WorkoutType, 45, 75);
      ...
      it('should validate workout types against expected values', () => {
        const validTypes: WorkoutType[] = [
          'recovery', 'easy', 'steady', 'tempo', 'threshold', 'vo2max', 'speed',
          'hill_repeats', 'fartlek', 'progression', 'race_pace', 'time_trial',
          'cross_training', 'strength', 'long_run'
        ];
    ...
    describe('Enhanced Integration with Zones Module', () => {
      it('should use zones module for intensity-to-zone mapping', () => {
        ...
        testIntensities.forEach(intensity => {
          ...
          const zoneExists = Object.values(TRAINING_ZONES).some(zone => 
            zone.name === workout.primaryZone.name
          );
      ...
      it('should maintain zone consistency across segments', () => {
        ...
        multiSegmentWorkout.segments.forEach(segment => {
          const zoneForIntensity = Object.values(TRAINING_ZONES).find(zone => {
          ...
          if (zoneForIntensity) {
            ...
            expect([segment.zone.name, zoneForIntensity.name]).toContain(segment.zone.name);
          }
        });
      });
      ...
      it('should handle zone boundary conditions', () => {
        ...
        boundaryTests.forEach(({ intensity, expectedZoneType }) => {
          ...
          const matchingZone = Object.values(TRAINING_ZONES).find(z => 
            z.name === workout.primaryZone.name
          );
    ...
    describe('TSS and Recovery Calculations', () => {
      it('should calculate TSS based on duration and intensity', () => {
        ...
        testCases.forEach(({ duration, intensity, expectedTSSRange }) => {
          ...
          expect(workout.estimatedTSS).toBeGreaterThanOrEqual(expectedTSSRange[0]);
      ...
      it('should calculate recovery time based on workout type and intensity', () => {
        ...
        recoveryTests.forEach(({ type, expectedRecovery }) => {
          const workout = createCustomWorkout(type, 45, 75);
          expect(workout.recoveryTime).toBeGreaterThanOrEqual(expectedRecovery[0]);
      ...
      it('should scale TSS with workout duration', () => {
        ...
        expect(doubleWorkout.estimatedTSS).toBeGreaterThan(baseWorkout.estimatedTSS * 1.5);
        expect(doubleWorkout.estimatedTSS).toBeLessThan(baseWorkout.estimatedTSS * 2.5);
      });
      ...
      it('should scale TSS with workout intensity', () => {
        ...
        expect(highIntensity.estimatedTSS).toBeGreaterThan(lowIntensity.estimatedTSS * 1.5);
      });
    });
  });
  ...
  describe('Extended Custom Workout Creation Tests', () => {
    describe('Complex Multi-Segment Workouts', () => {
      it('should create interval workout with warm-up, main set, and cool-down', () => {
        const intervalSegments = [
          { duration: 10, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Easy warm-up' },
          { duration: 5, intensity: 70, zone: TRAINING_ZONES.EASY, description: 'Build pace' },
          { duration: 3, intensity: 95, zone: TRAINING_ZONES.VO2_MAX, description: 'Hard interval' },
          { duration: 2, intensity: 50, zone: TRAINING_ZONES.RECOVERY, description: 'Recovery jog' },
          { duration: 3, intensity: 95, zone: TRAINING_ZONES.VO2_MAX, description: 'Hard interval' },
          { duration: 2, intensity: 50, zone: TRAINING_ZONES.RECOVERY, description: 'Recovery jog' },
          { duration: 3, intensity: 95, zone: TRAINING_ZONES.VO2_MAX, description: 'Hard interval' },
      ...
      it('should create pyramid workout with progressive intensity changes', () => {
        const pyramidSegments = [
          ...
          { duration: 5, intensity: 75, zone: TRAINING_ZONES.STEADY, description: 'Build 1' },
          { duration: 4, intensity: 85, zone: TRAINING_ZONES.TEMPO, description: 'Build 2' },
          { duration: 3, intensity: 90, zone: TRAINING_ZONES.THRESHOLD, description: 'Peak' },
          { duration: 4, intensity: 85, zone: TRAINING_ZONES.TEMPO, description: 'Descend 1' },
          { duration: 5, intensity: 75, zone: TRAINING_ZONES.STEADY, description: 'Descend 2' },
          { duration: 10, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Cool-down' }
        ];

        const workout = createCustomWorkout('progression', 41, 80, pyramidSegments);
        
        expect(workout.segments).toHaveLength(7);
        const totalDuration = pyramidSegments.reduce((sum, seg) => sum + seg.duration, 0);
        expect(totalDuration).toBe(41);
        
        // Verify intensity progression
        expect(workout.segments[3].intensity).toBe(90); // Peak intensity
        expect(workout.segments[0].intensity).toBeLessThan(workout.segments[3].intensity);
        expect(workout.segments[6].intensity).toBeLessThan(workout.segments[3].intensity);
      });
      ...
      it('should create fartlek workout with varied intensity segments', () => {
        const fartlekSegments = [
          ...
          { duration: 4, intensity: 65, zone: TRAINING_ZONES.EASY, description: 'Easy' },
          { duration: 0.5, intensity: 98, zone: TRAINING_ZONES.NEUROMUSCULAR, description: 'Sprint' },
          { duration: 2.5, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Recovery' },
    ...
    describe('Edge Cases and Error Handling', () => {
      it('should handle workouts with very short segments (sprints)', () => {
        const sprintSegments = [
          { duration: 5, intensity: 65, zone: TRAINING_ZONES.EASY, description: 'Warm-up' },
          { duration: 0.1, intensity: 100, zone: TRAINING_ZONES.NEUROMUSCULAR, description: '6 second sprint' },
          { duration: 1.9, intensity: 50, zone: TRAINING_ZONES.RECOVERY, description: 'Recovery' },
          { duration: 0.1, intensity: 100, zone: TRAINING_ZONES.NEUROMUSCULAR, description: '6 second sprint' },
          { duration: 1.9, intensity: 50, zone: TRAINING_ZONES.RECOVERY, description: 'Recovery' },
      ...
      it('should handle workouts with very long segments (ultra training)', () => {
        const ultraSegments = [
          { duration: 15, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Warm-up' },
          { duration: 240, intensity: 65, zone: TRAINING_ZONES.EASY, description: '4-hour steady run' },
          { duration: 15, intensity: 55, zone: TRAINING_ZONES.RECOVERY, description: 'Cool-down walk' }
        ];
      ...
      it('should handle segments with intensity outside normal ranges', () => {
        const extremeSegments = [
          { duration: 10, intensity: 40, zone: TRAINING_ZONES.RECOVERY, description: 'Walking warm-up' },
          { duration: 20, intensity: 110, zone: TRAINING_ZONES.NEUROMUSCULAR, description: 'Supramaximal effort' },
          { duration: 10, intensity: 35, zone: TRAINING_ZONES.RECOVERY, description: 'Walking cool-down' }
        ];
      ...
      it('should handle mismatched total duration and segment durations', () => {
        const segments = [
          { duration: 10, intensity: 65, zone: TRAINING_ZONES.EASY, description: 'Warm-up' },
          { duration: 30, intensity: 85, zone: TRAINING_ZONES.TEMPO, description: 'Main set' },
          { duration: 10, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Cool-down' }
        ];
        ...
        const actualDuration = workout.segments.reduce((sum, seg) => sum + seg.duration, 0);
        expect(actualDuration).toBe(50);
      });

      it('should handle negative or zero durations gracefully', () => {
        const invalidSegments = [
          { duration: -5, intensity: 65, zone: TRAINING_ZONES.EASY, description: 'Invalid negative' },
          { duration: 0, intensity: 85, zone: TRAINING_ZONES.TEMPO, description: 'Zero duration' },
          { duration: 10, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Valid segment' }
        ];
    ...
    describe('Workout Type Specific Tests', () => {
      ...
      it('should create appropriate race pace workout', () => {
        const racePaceSegments = [
          ...
          { duration: 5, intensity: 80, zone: TRAINING_ZONES.STEADY, description: 'Build' },
          { duration: 30, intensity: 88, zone: TRAINING_ZONES.THRESHOLD, description: 'Half marathon pace' },
          { duration: 10, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Cool-down' }
        ];

        const workout = createCustomWorkout('race_pace', 60, 80, racePaceSegments);
    ...
    describe('TSS and Recovery Calculation Edge Cases', () => {
      it('should calculate TSS for mixed intensity workout', () => {
        const mixedSegments = [
          { duration: 10, intensity: 50, zone: TRAINING_ZONES.RECOVERY, description: 'Walk' },
          { duration: 20, intensity: 95, zone: TRAINING_ZONES.VO2_MAX, description: 'Hard' },
          { duration: 10, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Recovery' }
        ];
      ...
      it('should calculate appropriate recovery time for different workout types', () => {
        const workoutTypeTests = [
          { type: 'recovery' as WorkoutType, duration: 30, intensity: 55, maxRecovery: 15 },
          { type: 'easy' as WorkoutType, duration: 60, intensity: 65, maxRecovery: 25 },
          { type: 'tempo' as WorkoutType, duration: 45, intensity: 85, maxRecovery: 40 },
          { type: 'threshold' as WorkoutType, duration: 40, intensity: 90, maxRecovery: 50 },
          { type: 'vo2max' as WorkoutType, duration: 35, intensity: 95, maxRecovery: 60 },
          { type: 'speed' as WorkoutType, duration: 30, intensity: 98, maxRecovery: 55 }
        ];

        workoutTypeTests.forEach(({ type, duration, intensity, maxRecovery }) => {
    ...
    describe('Zone Integration and Validation', () => {
      ...
      it('should maintain zone consistency when using custom segments', () => {
        const segments = [
          { duration: 10, intensity: 65, zone: TRAINING_ZONES.EASY, description: 'Warm-up' },
          { duration: 20, intensity: 88, zone: TRAINING_ZONES.THRESHOLD, description: 'Main' },
          { duration: 10, intensity: 60, zone: TRAINING_ZONES.RECOVERY, description: 'Cool-down' }
        ];
