...
describe("Pfitzinger Methodology Complete Test Suite", () => {
  ...
  describe("Requirement 3.1: Lactate Threshold-Based Pace System", () => {
    ...
    it("should derive all training paces from LT pace", () => {
      ...
      const paces = enhancedPlan.metadata
        ?.pfitzingerPaces as PfitzingerTrainingPaces;
    ...
    it("should calculate progressive threshold volume", () => {
      ...
      const thresholdProgression = enhancedPlan.metadata
        ?.thresholdVolumeProgression as number[];
      ...
      const hasVariation = thresholdProgression.some(
        (vol, i) => i > 0 && vol < thresholdProgression[i - 1],
      );
  ...
  describe("Requirement 3.2: Medium-Long Run Generation System", () => {
    it("should generate 12-16 mile medium-long runs", () => {
      const mediumLongWorkout = philosophy.selectWorkout(
        "long_run",
        "build",
        6,
      );
      ...
      const totalDuration = customized.segments.reduce(
    ...
    it("should include embedded tempo segments in medium-long runs", () => {
      ...
      const hasTempoSegment = customized.segments.some(
        (seg) => seg.intensity >= 82 && seg.intensity <= 88,
      );
    ...
    it("should progressively increase medium-long run difficulty", () => {
      ...
      expect(lateWorkout.estimatedTSS).toBeGreaterThanOrEqual(
        earlyWorkout.estimatedTSS,
      );
    });
  });
  ...
  describe("Requirement 3.3: Pfitzinger Weekly Structure System", () => {
    it("should implement day-of-week patterns", () => {
      const weeklyStructure = (philosophy as any).getPfitzingerBaseStructure(
        "build",
      );
      ...
      expect(["lactate_threshold", "tempo", "marathon_pace"]).toContain(
        thursdayType,
      );
    });
    ...
    it("should enforce 48-72 hour spacing between quality sessions", () => {
      ...
      expect(
        weeklyStructure.workoutSpacing.hardDaySpacing,
      ).toBeGreaterThanOrEqual(48);
      expect(weeklyStructure.workoutSpacing.hardDaySpacing).toBeLessThanOrEqual(
        72,
      );
    ...
    it("should calculate threshold volume progression", () => {
      const volumeProgression = (
        philosophy as any
      ).calculateThresholdVolumeProgression("build", 5);
      ...
      expect(
        volumeProgression.recoveryRequirements.hoursAfterLT,
      ).toBeGreaterThanOrEqual(24);
      expect(
        volumeProgression.recoveryRequirements.hoursBeforeLT,
      ).toBeGreaterThanOrEqual(24);
    });

    it("should integrate race-specific pace work", () => {
      const raceIntegration = (philosophy as any).getRaceSpecificIntegration(
        "peak",
        2,
      );
  ...
  describe("Requirement 3.4: Build Phase Threshold Volume", () => {
    it("should progressively increase threshold volume in build phase", () => {
      ...
      const volumes = weeks.map((week) => {
        const prog = (philosophy as any).calculateThresholdVolumeProgression(
          "build",
          week,
        );
  ...
  describe("Requirement 3.5: Tempo Run Variations", () => {
    it("should generate continuous tempo runs", () => {
      ...
      // Main tempo segment
      const tempoSegment = customized.segments.find(
        (seg) => seg.intensity >= 82 && seg.intensity <= 88,
      );
    ...
    it("should generate lactate threshold intervals", () => {
      ...
      const hasIntenseSegments = customized.segments.some(
  ...
  describe("Requirement 3.6: Race-Specific Pace Work", () => {
    it("should increase race pace work approaching goal race", () => {
      ...
      const earlyPeak = (philosophy as any).getRaceSpecificIntegration(
        "peak",
        1,
      );
      const latePeak = (philosophy as any).getRaceSpecificIntegration(
        "peak",
        4,
      );
      ...
      expect(latePeak.marathonPaceVolume).toBeGreaterThanOrEqual(
        earlyPeak.marathonPaceVolume - 5,
      ); // Allow some variation
    });

    it("should implement proper tapering protocols", () => {
      const taperStructure = (philosophy as any).getPfitzingerBaseStructure(
        "taper",
      );
      ...
      const hasQualityWork = Object.values(taperStructure.dayStructure).some(
        (day) => ["tempo", "race_pace", "lactate_threshold"].includes(day.type),
      );
  ...
  describe("Requirement 3.7: Weekly Structure Patterns", () => {
    ...
    it("should implement weekly variation to prevent monotony", () => {
      ...
      const week1 = (philosophy as any).generatePfitzingerWeeklyStructure(
        "build",
        4,
      );
      const week2 = (philosophy as any).generatePfitzingerWeeklyStructure(
        "build",
        5,
      );
  ...
  describe("Requirement 3.8: Training Load Calculations", () => {
    ...
    it("should implement recovery protocols", () => {
      const volumeProgression = (
        philosophy as any
      ).calculateThresholdVolumeProgression("build", 6);
  ...
  describe("Integration and Edge Cases", () => {
    ...
    it("should validate intensity distribution compliance", () => {
      ...
      expect(
        distribution.easy + distribution.moderate + distribution.hard,
      ).toBe(100);
    });

    it("should handle concurrent threshold and marathon pace work", () => {
      const peakStructure = (philosophy as any).getPfitzingerBaseStructure(
        "peak",
      );

      // Peak phase should balance both
      const hasThreshold = Object.values(peakStructure.dayStructure).some(
        (day) => day.type === "lactate_threshold" || day.thresholdMinutes,
      );
      const hasMarathonPace = Object.values(peakStructure.dayStructure).some(
        (day) => day.type === "marathon_pace" || day.marathonPaceMinutes,
      );
  ...
  describe("Performance and Optimization", () => {
    ...
    it("should handle plans of various lengths", () => {
      ...
      planLengths.forEach((weeks) => {
        ...
        expect(enhanced).toBeDefined();
        expect(enhanced.metadata?.thresholdVolumeProgression).toHaveLength(
          weeks,
        );
      });
    });
