...
function createMockEnvironmentalFactors(overrides?: Partial<DetailedEnvironmentalFactors>): DetailedEnvironmentalFactors {
  return {
    altitude: 500,
...
function createMockEquipmentConstraints(overrides?: Partial<EquipmentConstraints>): EquipmentConstraints {
  return {
...
function createMockTimeConstraints(overrides?: Partial<TimeConstraints>): TimeConstraints {
  return {
...
function createMockInjuryConstraints(overrides?: Partial<InjuryConstraints>): InjuryConstraints {
  return {
    currentInjuries: [],
    injuryHistory: [],
    painAreas: [],
...
describe('EnvironmentalConstraintAdapter', () => {
  ...
  describe('Environmental Adaptations (Requirement 6.3)', () => {
    it('should create altitude adaptations for moderate altitude', () => {
      ...
      expect(altitudeModification!.suggestedChanges.intensityReduction).toBeGreaterThan(0);

      const altitudeConstraint = result.constraints.environmental.find(constraint =>
        constraint.factor === 'altitude'
      );
    ...
    it('should create extreme altitude adaptations for high altitude', () => {
      ...
      const altitudeModifications = result.modifications.filter(mod => 
      ...
      const intensityMod = altitudeModifications.find(mod => mod.type === 'reduce_intensity');
      expect(intensityMod).toBeDefined();
      expect(intensityMod!.suggestedChanges.intensityReduction).toBeGreaterThan(15);

      // Should have phase adjustment for Lydiard
      const phaseMod = altitudeModifications.find(mod => mod.type === 'phase_adjustment');
    ...
    it('should create heat adaptations for hot weather', () => {
      ...
      const heatModifications = result.modifications.filter(mod => 
        mod.reason.includes('Heat') || mod.reason.includes('heat')
      );
      expect(heatModifications.length).toBeGreaterThanOrEqual(2);

      // Should reduce intensity
      const intensityMod = heatModifications.find(mod => mod.type === 'reduce_intensity');
      expect(intensityMod).toBeDefined();
      expect(intensityMod!.priority).toBe('high');

      // Should adjust timing
      const timingMod = heatModifications.find(mod => mod.type === 'workout_timing');
      expect(timingMod).toBeDefined();
      expect(timingMod!.suggestedChanges.avoidTimeWindows).toContain('10:00-16:00');

      const heatConstraint = result.constraints.environmental.find(constraint =>
        constraint.factor === 'heat_index'
      );
    ...
    it('should create cold weather adaptations', () => {
      ...
      const coldConstraint = result.constraints.environmental.find(constraint =>
        constraint.factor === 'cold_temperature'
      );
    ...
    it('should create terrain-specific adaptations', () => {
      ...
      const terrainModifications = result.modifications.filter(mod => 
        mod.reason.includes('terrain') || mod.reason.includes('Hill')
      );
      expect(terrainModifications.length).toBeGreaterThan(0);

      const substitutionMod = terrainModifications.find(mod => mod.type === 'workout_substitution');
      expect(substitutionMod).toBeDefined();
      expect(substitutionMod!.suggestedChanges.substituteWorkoutType).toBe('hill_repeats');

      // Lydiard should get phase enhancement
      const enhancementMod = terrainModifications.find(mod => mod.type === 'phase_enhancement');
    ...
    it('should create weather-specific adaptations for extreme conditions', () => {
      ...
      const windMod = result.modifications.find(mod => mod.reason.includes('wind'));
      expect(windMod).toBeDefined();

      // Should have precipitation adaptations
      const precipMod = result.modifications.find(mod => mod.reason.includes('precipitation'));
      ...
      const airQualityMod = result.modifications.find(mod => mod.reason.includes('air quality'));
  ...
  describe('Equipment Constraint Adaptations', () => {
    it('should adapt for limited surface availability', () => {
      ...
      const trackConstraint = result.constraints.equipment.find(constraint =>
        constraint.missing === 'running_track'
      );
      expect(trackConstraint).toBeDefined();
      expect(trackConstraint!.effectiveness).toBe(85);

      const gymConstraint = result.constraints.equipment.find(constraint =>
        constraint.missing === 'gym_access'
      );
    ...
    it('should adapt for missing safety equipment', () => {
      ...
      expect(safetyModification!.suggestedChanges.daylightOnlyRunning).toBe(true);

      // Should recommend safety equipment investment
      const safetyRecommendation = result.recommendations.find(rec =>
        rec.category === 'equipment' && rec.priority === 'critical'
      );
      expect(safetyRecommendation).toBeDefined();
      expect(safetyRecommendation!.recommendation).toContain('safety equipment');
    });
    ...
    it('should adapt for weather gear limitations', () => {
      ...
      expect(weatherGearModification!.suggestedChanges.indoorSeasonalOptions).toBe(true);
    });
  });
  ...
  describe('Time Constraint Adaptations (Requirement 6.8)', () => {
    it('should compress training for severe time constraints', () => {
      ...
      const timeCompressionMod = result.modifications.find(mod => 
      ...
      const durationConstraintMod = result.modifications.find(mod => 
        mod.type === 'duration_constraint'
      );
    ...
    it('should optimize for daily time slot limitations', () => {
      ...
      const timingModification = result.modifications.find(mod => 
        mod.type === 'workout_timing' && mod.reason.includes('Limited morning time')
      );
      expect(timingModification).toBeDefined();
      expect(timingModification!.suggestedChanges.morningWorkoutTypes).toContain('easy_short');
      expect(timingModification!.suggestedChanges.qualityWorkoutsToOtherSlots).toBe(true);
    });
    ...
    it('should create different compression strategies based on time deficit', () => {
      ...
      const moderateCompression = moderateResult.constraints.time[0]?.compression;
    ...
    it('should provide time management recommendations', () => {
      ...
      const timeRecommendation = result.recommendations.find(rec =>
        rec.category === 'scheduling'
      );
      expect(timeRecommendation).toBeDefined();
      expect(timeRecommendation!.recommendation).toContain('high-intensity, low-volume');
    });
  });
  ...
  describe('Injury Constraint Adaptations (Requirement 6.6)', () => {
    it('should adapt for current injuries', () => {
      ...
      const injuryModification = result.modifications.find(mod => 
        mod.type === 'injury_accommodation'
      );
      ...
      expect(injuryModification!.suggestedChanges.avoidActivities).toContain('no_running');
    ...
    it('should adapt for injury history', () => {
      ...
      const preventionModifications = result.modifications.filter(mod => 
        mod.type === 'injury_prevention'
      );
    ...
    it('should adapt for high-risk factors', () => {
      const highRiskConstraints = createMockInjuryConstraints({
        riskFactors: [{
          ...
          severity: 'high',
          mitigationStrategies: ['orthotics', 'strength_training', 'gait_analysis']
        }]
      });
      ...
      const riskMitigation = result.modifications.find(mod => 
        mod.type === 'risk_mitigation'
      );
      expect(riskMitigation).toBeDefined();
      expect(riskMitigation!.priority).toBe('high');
      expect(riskMitigation!.suggestedChanges.mitigationStrategies).toContain('orthotics');

      // Should recommend injury prevention program
      const preventionRecommendation = result.recommendations.find(rec =>
        rec.category === 'injury_prevention'
      );
    ...
    it('should assess dynamic injury risk from workout history', () => {
      ...
      completedWorkouts.forEach((workout, index) => {
        if (index < 5) {
          workout.actualMetrics!.duration = 90; // Recent high load
        }
      });
  ...
  describe('Comprehensive Adaptation Integration', () => {
    it('should handle multiple constraint types simultaneously', () => {
      ...
      expect(result.recommendations.some(r => r.category === 'environmental')).toBe(true);
      expect(result.recommendations.some(r => r.category === 'equipment')).toBe(true);
    ...
    it('should resolve conflicting adaptations appropriately', () => {
      ...
      const intensityReductions = result.modifications.filter(mod => 
        mod.type === 'reduce_intensity'
      );
      const intensityIncreases = result.modifications.filter(mod => 
        mod.type === 'increase_intensity' || 
        (mod.type === 'time_compression' && mod.suggestedChanges.intensityCompensation)
      );
      ...
      const timeModifications = result.modifications.filter(mod => 
    ...
    it('should provide methodology-specific adaptations', () => {
      ...
      const lydiardHasAerobic = lydiardResult.modifications.some(mod => 
        mod.reason.includes('aerobic') || mod.reason.includes('Lydiard')
      );

      // Daniels should have VDOT-related adaptations
      expect(danielsHasVDOT || danielsResult.recommendations.some(r => r.recommendation.includes('VDOT'))).toBe(true);
    ...
    it('should calculate adaptation effectiveness accurately', () => {
      ...
      const heavyConstraints = adapter.adaptPlan(
        mockPlan,
        'daniels',
        createMockEnvironmentalFactors({ altitude: 4000, typicalTemperature: 35 }),
        createMockEquipmentConstraints({ availableSurfaces: ['road'], hasGym: false }),
        createMockTimeConstraints({ weeklyAvailableHours: 3 }),
        createMockInjuryConstraints({ 
          currentInjuries: [{ 
            type: 'knee', severity: 'moderate', stage: 'healing', 
            restrictions: ['no_running'], expectedRecovery: 4 
          }] 
        })
      );
  ...
  describe('Risk Assessment and Monitoring', () => {
    it('should assess overall risk levels appropriately', () => {
      ...
      const injuryRisk = result.riskAssessment.specificRisks.find(risk => 
        risk.type === 'injury'
      );
    ...
    it('should provide appropriate monitoring points', () => {
      const complexScenario = adapter.adaptPlan(
        ...
        createMockEquipmentConstraints({ safetyEquipment: { reflectiveGear: false, lights: false, emergencyDevice: false } }),
      ...
      expect(monitoringPoints.some(point => point.includes('injury'))).toBe(true);
      expect(monitoringPoints.some(point => point.includes('load'))).toBe(true);
    });
  });
