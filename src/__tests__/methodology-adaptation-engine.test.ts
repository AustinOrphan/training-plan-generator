...
describe("MethodologyAdaptationEngine", () => {
  ...
  describe("analyzeProgressWithMethodology", () => {
    it("should analyze progress with methodology context", () => {
      ...
      expect(
        result.methodologyInsights.philosophyAlignment,
      ).toBeGreaterThanOrEqual(0);
      expect(
        result.methodologyInsights.philosophyAlignment,
      ).toBeLessThanOrEqual(100);
      expect(
        Array.isArray(result.methodologyInsights.adaptationRecommendations),
      ).toBe(true);
    ...
    it("should handle plans without methodology", () => {
      ...
      expect(result.methodologyInsights.responseProfileStatus).toBe(
        "no_methodology",
      );
    });
    ...
    it("should provide methodology-specific recommendations", () => {
      ...
      expect(
        result.methodologyInsights.adaptationRecommendations,
      ).toBeDefined();
      expect(
        Array.isArray(result.methodologyInsights.adaptationRecommendations),
      ).toBe(true);
  ...
  describe("suggestMethodologyAwareModifications", () => {
    ...
    it("should prioritize modifications based on methodology principles", () => {
      ...
      if (modifications.length > 1) {
        ...
        const hasMethodologySpecific = modifications.some(
          (m) => m.methodologySpecific,
        );

        if (hasMethodologySpecific) {
          const methodologyMods = modifications.filter(
            (m) => m.methodologySpecific,
          );
          const generalMods = modifications.filter(
            (m) => !m.methodologySpecific,
          );

          if (methodologyMods.length > 0 && generalMods.length > 0) {
            const firstMethodologyIndex = modifications.indexOf(
              methodologyMods[0],
            );
  ...
  describe("Integration with Base Adaptation Engine", () => {
    it("should extend base functionality without breaking existing features", () => {
      const baseResult = engine.analyzeProgress(
        completedWorkouts,
        plannedWorkouts,
      );
      ...
      expect(enhancedResult.completedWorkouts).toBe(
        baseResult.completedWorkouts,
      );
    ...
    it("should provide both base and methodology-specific modifications", () => {
      const baseModifications = engine.suggestModifications(
        testPlan,
      ...
      const hasMethodologySpecific = enhancedModifications.some(
        (mod) => mod.methodologySpecific,
      );
      if (enhancedModifications.length > 0) {
        expect(
          enhancedModifications.every((mod) =>
            mod.hasOwnProperty("methodologySpecific"),
          ),
        ).toBe(true);
      }
    });
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle empty workout arrays", () => {
      const result = engine.analyzeProgressWithMethodology([], [], testPlan);
  ...
  describe("analyzeAdaptationNeeds", () => {
    ...
    it("should work with different methodologies", () => {
      ...
      methodologies.forEach((methodology) => {
        ...
        const methodologyGenerator = new AdvancedTrainingPlanGenerator(
          methodologyConfig,
        );
  ...
  describe("getModificationSuggestions", () => {
    ...
    it("should handle different progress scenarios", () => {
      ...
      scenarios.forEach((scenario) => {
        const suggestions =
          MethodologyAdaptationUtils.getModificationSuggestions(
            testPlan,
            scenario.progress,
            "daniels",
          );
...
describe("Methodology-Specific Adaptation Patterns", () => {
  ...
  it("should have distinct patterns for each methodology", () => {
    ...
    methodologies.forEach((methodology) => {
      ...
      if (result.methodologyInsights.adaptationRecommendations.length > 0) {
        expect(
          result.methodologyInsights.adaptationRecommendations.every(
  ...
  it("should provide methodology-appropriate modification suggestions", () => {
    ...
    methodologies.forEach((methodology) => {
      ...
      const methodologySpecific = modifications.filter(
        (mod) => mod.methodologySpecific,
      );
