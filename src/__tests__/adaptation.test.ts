...
describe("SmartAdaptationEngine", () => {
  ...
  describe("Progress Analysis", () => {
    ...
    it("should identify overreaching patterns", () => {
      ...
      const needsChange = adaptationEngine.needsAdaptation(
        progressData,
        recoveryMetrics,
      );
  ...
  describe("Recovery Monitoring", () => {
    ...
    it("should identify fatigue accumulation", () => {
      ...
      const needsIntervention = adaptationEngine.needsAdaptation(
        progressData,
        fatigueRecovery,
      );
  ...
  describe("Plan Modification", () => {
    ...
    it("should suggest intensity reduction for overreaching", () => {
      ...
      const intensityMod = modifications.find(
    ...
    it("should recommend progressive loading for good adaptation", () => {
      ...
      completedWorkouts.forEach((workout) => {
        ...
        workout.actualDuration =
          (workout.plannedWorkout?.targetMetrics?.duration || 45) * 0.95; // 5% faster based on planned duration
      });
    ...
    it("should provide recovery protocols for injury risk", () => {
      ...
      const injuryMod = modifications.find(
        (m) => m.type === "injury_protocol" || m.type === "add_recovery",
      );
      if (injuryMod) {
  ...
  describe("Workout Adaptation via Modifications", () => {
    it("should suggest workout substitution for poor recovery", () => {
      ...
      const substituteMod = modifications.find(
        (m) => m.type === "substitute_workout",
      );
      if (substituteMod) {
        expect(
          substituteMod.suggestedChanges.substituteWorkoutType,
        ).toBeDefined();
  ...
  describe("Injury Risk Assessment", () => {
    ...
    it("should identify rapid training load increases", () => {
      const rapidIncreaseHistory = Array(8)
        .fill(null)
        .map((_, week) => ({
      ...
      const mockTrainingLoad = {
        ...
        ratio: 1.67,
        trend: "increasing" as const,
        recommendation: "High risk - reduce load",
      };
  ...
  describe("Adaptation Settings", () => {
    it("should work with different engine instances", () => {
      ...
      const completedWorkouts = generateCompletedWorkouts(1, 3);
      const analysis = engine1.analyzeProgress(
        completedWorkouts,
        mockPlan.workouts,
      );
    ...
    it("should enforce maximum adjustment limits", () => {
      ...
      modifications.forEach((mod) => {
        if (
          mod.type === "reduce_volume" &&
          mod.suggestedChanges.volumeReduction
        ) {
          expect(mod.suggestedChanges.volumeReduction).toBeLessThanOrEqual(100);
        }
        if (
          mod.type === "reduce_intensity" &&
          mod.suggestedChanges.intensityReduction
        ) {
          expect(mod.suggestedChanges.intensityReduction).toBeLessThanOrEqual(
            100,
          );
        }
      });
  ...
  describe("Real-world Scenarios", () => {
    ...
    it("should handle plateau breaking scenario", () => {
      ...
      const changeMod = modifications.find(
        (m) =>
