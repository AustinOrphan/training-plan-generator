...
describe("AdvancedTrainingPlanGenerator", () => {
  ...
  describe("Constructor and Initialization", () => {
    ...
    it("should default to custom methodology if not specified", () => {
      ...
      const defaultGenerator = new AdvancedTrainingPlanGenerator(
        configWithoutMethodology,
      );
  ...
  describe("generateAdvancedPlan", () => {
    ...
    it("should respect target race date", () => {
      ...
      expect(lastWorkout.date.getTime()).toBeLessThanOrEqual(
        mockConfig.targetDate.getTime(),
      );
    });
    ...
    it("should generate workouts following philosophy intensity distribution", () => {
      ...
      validateIntensityDistribution(
        plan.workouts,
        mockConfig.intensityDistribution || {
          easy: 80,
          moderate: 15,
          hard: 4,
          veryHard: 1,
  ...
  describe("Multi-Race Planning", () => {
    ...
    it("should prioritize A races over B races", () => {
      ...
      const race1Workouts = plan.workouts.filter(
        (w) =>
          Math.abs(w.date.getTime() - race1Date.getTime()) <
          7 * 24 * 60 * 60 * 1000,
      );

      const race2Workouts = plan.workouts.filter(
        (w) =>
          Math.abs(w.date.getTime() - race2Date.getTime()) <
          7 * 24 * 60 * 60 * 1000,
      );
      ...
      const race2TotalTSS = race2Workouts.reduce(
        (sum, w) => sum + w.targetMetrics.tss,
        0,
      );
      const race1TotalTSS = race1Workouts.reduce(
        (sum, w) => sum + w.targetMetrics.tss,
        0,
      );
    ...
    it("should create transition periods between races", () => {
      ...
      const postRace1Workouts = plan.workouts.filter(
        (w) =>
  ...
  describe("Periodization Handling", () => {
    it("should apply linear periodization", () => {
      ...
      const validBlocks = plan.blocks.filter(
        (b) => b.microcycles && b.microcycles.length > 0,
      );
      expect(validBlocks.length).toBeGreaterThan(0);

      // Validate total training load is reasonable across all blocks
      const totalLoad = plan.blocks.reduce(
        (sum, b) =>
          sum +
          (b.microcycles?.reduce((s, m) => s + (m.totalLoad || 0), 0) || 0),
        0,
      );
  ...
  describe("Integration with Base Generator", () => {
    ...
    it("should maintain deterministic generation", () => {
      ...
      expect(
        Math.abs(plan1.blocks.length - plan2.blocks.length),
  ...
  describe("Configuration Validation", () => {
    it("should generate plan that meets methodology requirements", () => {
      ...
      const easyWorkouts = intensityTypes.filter((t) =>
        ["easy", "recovery"].includes(t),
      ).length;
