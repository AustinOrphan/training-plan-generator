...
describe('Constants Module', () => {
  ...
  describe('PROGRESSION_RATES', () => {
    it('should have safe weekly progression rates', () => {
      ...
      expect(PROGRESSION_RATES.advanced).toBe(0.10); // 10%
      expect(PROGRESSION_RATES.maxSingleWeek).toBe(0.20); // 20%
    });

    it('should have logical progression hierarchy', () => {
      expect(PROGRESSION_RATES.beginner).toBeLessThan(PROGRESSION_RATES.intermediate);
      expect(PROGRESSION_RATES.intermediate).toBeLessThan(PROGRESSION_RATES.advanced);
      expect(PROGRESSION_RATES.advanced).toBeLessThan(PROGRESSION_RATES.maxSingleWeek);
    });
  ...
  describe('RECOVERY_MULTIPLIERS', () => {
    ...
    it('should have logical intensity-recovery relationship', () => {
      ...
      expect(RECOVERY_MULTIPLIERS.recovery).toBeLessThan(RECOVERY_MULTIPLIERS.easy);
      expect(RECOVERY_MULTIPLIERS.easy).toBeLessThan(RECOVERY_MULTIPLIERS.steady);
      expect(RECOVERY_MULTIPLIERS.steady).toBeLessThan(RECOVERY_MULTIPLIERS.tempo);
      expect(RECOVERY_MULTIPLIERS.tempo).toBeLessThan(RECOVERY_MULTIPLIERS.threshold);
      expect(RECOVERY_MULTIPLIERS.vo2max).toBeGreaterThan(RECOVERY_MULTIPLIERS.threshold);
      expect(RECOVERY_MULTIPLIERS.race).toBeGreaterThan(RECOVERY_MULTIPLIERS.vo2max);
    });
  ...
  describe('WORKOUT_DURATIONS', () => {
    ...
    it('should maintain logical duration hierarchy', () => {
      expect(WORKOUT_DURATIONS.recovery.typical)
        .toBeLessThan(WORKOUT_DURATIONS.easy.typical);
      expect(WORKOUT_DURATIONS.easy.typical)
        .toBeLessThan(WORKOUT_DURATIONS.long_run.typical);
    });
  });
  ...
  describe('TRAINING_METHODOLOGIES', () => {
    it('should include all major methodologies', () => {
      const expectedMethodologies = ['daniels', 'lydiard', 'pfitzinger', 'hudson', 'custom'];
    ...
    it('should have complete phase transitions', () => {
      Object.values(TRAINING_METHODOLOGIES).forEach(methodology => {
        ...
        phases.forEach(phase => {
          ...
          expect(methodology.phaseTransitions[phase]).toHaveProperty('duration');
  ...
  describe('WORKOUT_EMPHASIS', () => {
    it('should have emphasis values for all methodologies', () => {
      const methodologies = ['daniels', 'lydiard', 'pfitzinger', 'hudson', 'custom'];
  ...
  describe('METHODOLOGY_PHASE_TARGETS', () => {
    it('should have targets for all methodologies and phases', () => {
      ...
      methodologies.forEach(method => {
        ...
        phases.forEach(phase => {
          ...
          expect(Array.isArray(METHODOLOGY_PHASE_TARGETS[method][phase])).toBe(true);
          expect(METHODOLOGY_PHASE_TARGETS[method][phase].length).toBeGreaterThan(0);
        });
      });
    });
    ...
    it('should have consistent taper targets', () => {
      Object.values(METHODOLOGY_PHASE_TARGETS).forEach(methodology => {
        ...
        expect(taperTargets.some(target => 
    ...
    it('should have base phase aerobic emphasis', () => {
      Object.values(METHODOLOGY_PHASE_TARGETS).forEach(methodology => {
        ...
        expect(baseTargets.some(target => 
          target.includes('aerobic') || target.includes('mitochondrial') || target.includes('capillar')
        )).toBe(true);
      });
    });
  ...
  describe('Constant Relationships and Cross-Validation', () => {
    it('should have consistent adaptation and phase relationships', () => {
      ...
      expect(PHASE_DURATION.base.optimal * 7).toBeGreaterThanOrEqual(ADAPTATION_TIMELINE.aerobic_capacity);
      expect(PHASE_DURATION.base.max * 7).toBeGreaterThanOrEqual(ADAPTATION_TIMELINE.mitochondrial);
    });
