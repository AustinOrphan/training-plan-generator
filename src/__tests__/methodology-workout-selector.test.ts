...
import { MethodologyWorkoutSelector, WorkoutSelectionCriteria } from '../methodology-workout-selector';
import { 
...
describe('MethodologyWorkoutSelector', () => {
  ...
  describe('Pfitzinger Methodology Selector', () => {
    ...
    it('should provide LT interval alternatives', () => {
      ...
      expect(result.workout.adaptationTarget.toLowerCase()).toMatch(/threshold|lactate|tempo/);
    });
  });
  ...
  describe('Context-Aware Modifications', () => {
    ...
    it('should adjust for altitude', () => {
      ...
      expect(result.warnings?.some(w => w.toLowerCase().includes('altitude'))).toBe(true);
    });
    ...
    it('should adjust for high temperature', () => {
      ...
      const tempoSegment = result.workout.segments.find(seg => seg.zone.name === 'THRESHOLD');
      if (tempoSegment) {
    ...
    it('should modify speed work for non-track surfaces', () => {
      ...
      expect(result.warnings?.some(w => w.toLowerCase().includes('non-track'))).toBe(true);
  ...
  describe('Workout Progression', () => {
      ...
      const previousWorkout = createMockPlannedWorkout({
        ...
        workout: {
          ...
          segments: [
            ...
            { duration: 20, intensity: 88, zone: { name: 'THRESHOLD' }, description: 'Threshold' },
    ...
    it('should not progress if difficulty rating is too high', () => {
      ...
      const result = selector.getProgressedWorkout(
        previousWorkout,
        10,
        { completionRate: 0.92, difficultyRating: 9 }
      );
  ...
  describe('Alternative Options', () => {
    it('should provide methodology-specific alternatives', () => {
      ...
      expect(result.alternativeOptions?.some(opt => opt.includes('I-pace'))).toBe(true);
    });
    ...
    it('should provide Pfitzinger medium-long alternatives', () => {
      ...
      expect(result.alternativeOptions?.some(opt => opt.includes('Medium-long'))).toBe(true);
      expect(result.alternativeOptions?.some(opt => opt.includes('Progressive'))).toBe(true);
    });
  });
});