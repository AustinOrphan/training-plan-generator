...
describe("End-to-End Integration Tests", () => {
  ...
  describe("Multi-Philosophy Comparison", () => {
    ...
    it("should generate distinct plans for different philosophies", async () => {
      ...
      const distributions = testPhilosophies.map((methodology) => {
        ...
        validateIntensityDistribution(
          plan.workouts,
          philosophy.intensityDistribution,
        );

        return {
          methodology,
          easyWorkouts: plan.workouts.filter((w: PlannedWorkout) =>
            ["recovery", "easy"].includes(w.type),
          ).length,
          hardWorkouts: plan.workouts.filter((w: PlannedWorkout) =>
      ...
      const pfitzinger = distributions.find(
        (d) => d.methodology === "pfitzinger",
      )!;

      expect(lydiard.easyWorkouts).toBeGreaterThanOrEqual(
        pfitzinger.easyWorkouts,
      );
    });
    ...
    it("should maintain consistent plan quality across philosophies", async () => {
      ...
      for (const methodology of testPhilosophies) {
        ...
        const weeklyDistances = plan.blocks.flatMap((block: TrainingBlock) =>
          block.microcycles.map(
            (cycle: WeeklyMicrocycle) => cycle.totalDistance,
          ),
        );

        const firstHalf = weeklyDistances.slice(
          0,
          Math.floor(weeklyDistances.length / 2),
        );
        const secondHalf = weeklyDistances.slice(
          Math.floor(weeklyDistances.length / 2),
        );
  ...
  describe("Multi-Race Season Planning", () => {
    it("should handle complex season with multiple race priorities", async () => {
      ...
      const majorRaceCount = seasonConfig.targetRaces!.filter(
        (r) => r.priority === "A",
      ).length;
      expect(seasonPlan.blocks.length).toBeGreaterThanOrEqual(
        majorRaceCount * 2,
      ); // Build + peak for each A race

      // Should show progression toward marathon (longest distance)
      const marathonDate = seasonConfig.targetRaces!.find(
        (r) => r.distance === "marathon",
      )!.date;
      const marathonWeek = testDateUtils.calculateWeeks(
        seasonConfig.startDate,
        marathonDate,
      );

      const longRuns = seasonPlan.workouts.filter(
        (w: PlannedWorkout) => w.type === "long_run",
      );
      const marathonPrepLongRuns = longRuns.filter((w: PlannedWorkout) => {
        const workoutWeek = testDateUtils.calculateWeeks(
          seasonConfig.startDate,
          w.date,
        );
        return (
          workoutWeek >= marathonWeek - 16 && workoutWeek <= marathonWeek - 3
    ...
    it("should balance competing demands of different race distances", async () => {
      ...
      const speedWorkouts = plan.workouts.filter((w: PlannedWorkout) =>
        ["speed", "vo2max"].includes(w.type),
      );
      const enduranceWorkouts = plan.workouts.filter((w: PlannedWorkout) =>
        ["long_run", "steady"].includes(w.type),
      );

      // Should have more endurance emphasis due to marathon priority
      expect(enduranceWorkouts.length).toBeGreaterThanOrEqual(
        speedWorkouts.length,
      );
    });
  });
  ...
  describe("Real-World Scenario Testing", () => {
    it("should handle beginner to intermediate progression", async () => {
      ...
      const lastBuildWeek =
        weeklyVolumes[Math.floor(weeklyVolumes.length * 0.8)]; // Before taper
    ...
    it("should handle experienced athlete with high volume", async () => {
      ...
      expect(plan.summary.peakWeeklyDistance).toBeGreaterThan(
      ...
      expect(qualityWorkouts.length).toBeGreaterThan(
        plan.workouts.length * 0.15,
      ); // At least 15% quality workouts
    });
    ...
    it("should handle injury recovery return-to-running", async () => {
      ...
      const firstWeekWorkouts = plan.workouts.filter((w: PlannedWorkout) => {
        const weeksDiff = testDateUtils.calculateWeeks(
      ...
      expect(easyFirstWeek.length).toBeGreaterThanOrEqual(
        Math.floor(firstWeekWorkouts.length * 0.7),
      ); // At least 70% easy workouts for recovery

      // Should have gradual progression
      const firstBlockAvg =
        plan.blocks[0].microcycles.reduce(
          (sum: number, cycle: WeeklyMicrocycle) => sum + cycle.totalDistance,
          0,
  ...
  describe("Performance and Scalability", () => {
    it("should handle concurrent plan generation", async () => {
      const configs = Array(5)
        .fill(null)
        .map(() => createMockAdvancedPlanConfig());
