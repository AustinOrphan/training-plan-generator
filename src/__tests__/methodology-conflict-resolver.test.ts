...
function createMockAdvancedPlanConfig(
...
describe("MethodologyConflictResolver", () => {
  ...
  describe("Core Conflict Resolution (Requirement 6.4)", () => {
    it("should identify intensity distribution conflicts for Daniels methodology", () => {
      ...
      const intensityConflict = result.conflicts.find(
    ...
    it("should identify volume progression conflicts", () => {
      const config = createMockAdvancedPlanConfig({
        volume: { progressionRate: 0.2 }, // Very high progression rate
      });

      const result = resolver.resolveConflicts(config, "daniels");

      const volumeConflict = result.conflicts.find(
        (c) => c.type === "volume_violation",
      );
    ...
    it("should identify safety violations for high volume + high intensity", () => {
      ...
      const safetyConflict = result.conflicts.find(
        (c) => c.type === "safety_violation",
      );
    ...
    it("should identify recovery emphasis conflicts", () => {
      ...
      const recoveryConflict = result.conflicts.find(
        (c) => c.type === "recovery_violation",
      );
  ...
  describe("Resolution Strategy Generation", () => {
    it("should recommend safety override for critical conflicts", () => {
      ...
      const criticalResolution = result.resolutions.find(
        (r) => r.strategy === "safety_override",
      );
    ...
    it("should recommend strict methodology for high severity principle violations", () => {
      ...
      const strictResolution = result.resolutions.find(
        (r) => r.strategy === "strict_methodology",
      );
    ...
    it("should recommend guided modification for medium severity conflicts", () => {
      ...
      const guidedResolution = result.resolutions.find(
        (r) => r.strategy === "guided_modification",
      );
  ...
  describe("Safe Customization Options (Requirement 6.5)", () => {
    it("should provide safe customization options for each methodology", () => {
      ...
      const pfitzingerOptions =
        resolver.getSafeCustomizationOptions("pfitzinger");
  ...
  describe("Methodology Principle Preservation", () => {
    it("should check principle preservation for Daniels methodology", () => {
      ...
      const preservation = resolver.checkPrinciplePreservation(
        config,
        "daniels",
      );
      ...
      expect(preservation.integrityLevel).toMatch(
    ...
    it("should determine appropriate integrity levels", () => {
      ...
      const goodPreservation = resolver.checkPrinciplePreservation(
        goodConfig,
        "daniels",
      );
      ...
      const badPreservation = resolver.checkPrinciplePreservation(
        badConfig,
        "daniels",
      );
  ...
  describe("Safety Assessment", () => {
    it("should assess overall safety correctly", () => {
      ...
      expect(result.overallSafety.recommendedAction).toMatch(
        /abort|reconsider|modify|proceed/,
      );
    });
  ...
  describe("Customization Validation", () => {
    it("should validate customizations against methodology principles", () => {
      ...
      const validation = resolver.validateCustomization(
        customization,
        "lydiard",
      );
  ...
  describe("Methodology-Specific Behavior", () => {
    it("should apply Daniels-specific conflict detection", () => {
      ...
      expect(
        result.conflicts.some((c) => c.description.includes("80/20")),
      ).toBe(true);
    });
    ...
    it("should apply Lydiard-specific conflict detection", () => {
      ...
      expect(result.conflicts.some((c) => c.description.includes("85%"))).toBe(
        true,
      );
    });
    ...
    it("should provide methodology-specific safe customizations", () => {
      ...
      expect(
        danielsOptions.restrictions.some(
          (r) =>
            r.methodologyPrinciple.includes("Daniels") ||
            r.methodologyPrinciple.includes("80/20"),
        ),
      ).toBe(true);

      expect(
        lydiardOptions.restrictions.some(
          (r) =>
            r.methodologyPrinciple.includes("Lydiard") ||
            r.methodologyPrinciple.includes("Aerobic"),
        ),
      ).toBe(true);
    });
  });
