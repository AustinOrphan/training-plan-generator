...
import { calculateFitnessMetrics, calculateVDOT, calculateCriticalSpeed } from '../calculator';
...
describe('Performance Optimization and Validation', () => {
  describe('Plan Generation Performance', () => {
    ...
    it('should scale linearly with plan complexity', async () => {
      ...
      const performanceData: Array<{ weeks: number; time: number; workouts: number }> = [];
      ...
      timePerWeek.forEach(tpw => {
        expect(Math.abs(tpw - avgTimePerWeek)).toBeLessThan(avgTimePerWeek * 1.2); // Allow for fixed overhead
      });
    });
  });
  ...
  describe('Memory Usage Optimization', () => {
    ...
    it('should efficiently handle large data structures', async () => {
      const largeConfig = createMockAdvancedPlanConfig({
        targetRaces: Array(10).fill(null).map((_, i) => 
          createMockTargetRace({
      ...
      const memoryPerWorkout = (peakMemory - initialMemory) / plan.workouts.length;
  ...
  describe('Scientific Accuracy Validation', () => {
    it('should maintain VDOT calculation accuracy within 10% margin', () => {
      ...
      const testCases = [
        { runs: [{ date: new Date(), distance: 5, duration: 20, avgPace: 4.0, effortLevel: 9 }], expectedVDOT: 58, tolerance: 50 },
        { runs: [{ date: new Date(), distance: 10, duration: 45, avgPace: 4.5, effortLevel: 9 }], expectedVDOT: 52, tolerance: 50 },
        { runs: [{ date: new Date(), distance: 5, duration: 25, avgPace: 5.0, effortLevel: 9 }], expectedVDOT: 45, tolerance: 50 }
      ];
      
      testCases.forEach(({ runs, expectedVDOT, tolerance }) => {
        const calculatedVDOT = calculateVDOT(runs as any);
        const errorPercentage = Math.abs(calculatedVDOT - expectedVDOT) / expectedVDOT * 100;
    ...
    it('should validate TSS calculations against known values', () => {
      ...
      testWorkouts.forEach(({ duration, pace, thresholdPace, expectedTSS }) => {
        ...
        const calculatedTSS = (duration * Math.pow(intensityFactor, 2) * 100) / 60;
        
        const errorPercentage = Math.abs(calculatedTSS - expectedTSS) / expectedTSS * 100;
    ...
    it('should validate periodization distribution', async () => {
      ...
      // Count workout types
      const workoutCounts = plan.workouts.reduce((counts: Record<string, number>, workout: any) => {
      ...
      const hardWorkouts = (workoutCounts.threshold || 0) + (workoutCounts.vo2max || 0) + (workoutCounts.speed || 0);
  ...
  describe('Adaptation Engine Performance', () => {
    it('should optimize progress analysis calculations', async () => {
      ...
      const { time } = await measureExecutionTime(async () => {
        return adaptationEngine.analyzeProgress(completedWorkouts, plannedWorkouts);
      });
  ...
  describe('Deterministic Behavior Validation', () => {
    it('should generate similar plans for identical inputs', async () => {
      ...
      expect(Math.abs(plan1.workouts.length - plan2.workouts.length)).toBeLessThan(5);
      expect(plan1.blocks.length).toBe(plan2.blocks.length);
      expect(Math.abs(plan1.summary.totalWeeks - plan2.summary.totalWeeks)).toBeLessThan(2);
    ...
    it('should maintain calculation consistency across runs', () => {
      ...
      const results = Array(10).fill(null).map(() => calculateVDOT(runHistory));
