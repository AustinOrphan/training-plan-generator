...
export class MethodologyConflictResolver {
  ...
  public resolveConflicts(
    ...
    const safeCustomizations = this.determineSafeCustomizations(
      methodology,
      config,
    );
  ...
  public checkPrinciplePreservation(
    ...
    const intensityPreservation = this.checkIntensityDistribution(
      config,
      methodology,
    );
    ...
    const recoveryPreservation = this.checkRecoveryEmphasis(
      config,
      methodology,
    );
    ...
    const workoutPreservation = this.checkWorkoutPriorities(
      config,
      methodology,
    );
    ...
    const integrityLevel = this.determineIntegrityLevel(
      preservationScore,
      violations,
    );
    ...
    if (methodology === "daniels") {
      ...
      if (intensityConfig && intensityConfig.easy < 75) {
        conflicts.push({
          ...
          severity: "medium",
          description:
            "Daniels methodology recommends 80/20 easy/hard distribution",
          affectedAspect: "intensity_distribution",
          currentValue: intensityConfig.easy || "unknown",
          suggestedValue: "80%",
  ...
  private checkVolumeConflicts(
    ...
    // Use default progression rates from constants since methodologyConstants doesn't have progressionRate
    const maxProgressionRate = 0.1; // Default to 10% weekly increase
    const volumeConfig = getVolumeConfig(config);
    if (
      volumeConfig &&
      volumeConfig.progressionRate > maxProgressionRate * 1.5
    ) {
      conflicts.push({
  ...
  private checkRecoveryConflicts(
    ...
    if (
      recoveryConfig &&
      recoveryConfig.emphasis < expectedRecoveryEmphasis - 0.1
    ) {
      conflicts.push({
  ...
  private generateResolution(
    ...
    return {
      ...
      effectivenessImpact: this.calculateEffectivenessImpact(
        conflict,
        strategy,
      ),
  ...
  private determineResolutionStrategy(
    ...
    if (
      ...
      conflict.type === "safety_violation"
    ) {
  ...
  private calculateEffectivenessImpact(
    conflict: MethodologyConflict,
    strategy: ResolutionStrategy,
  ): number {
    const baseImpact =
      conflict.impacts.reduce(
        (sum, impact) => sum + impact.effectivenessReduction,
        0,
      ) / conflict.impacts.length;

    switch (strategy) {
      case "safety_override":
        return baseImpact * 0.3; // Safety first, effectiveness second
      case "strict_methodology":
        return baseImpact * 0.1; // Minimal impact
      case "guided_modification":
        return baseImpact * 0.5; // Moderate impact
      case "progressive_adaptation":
        return baseImpact * 0.7; // Higher impact initially
      default:
        return baseImpact;
    }
  }
   ...
   */
  private checkIntensityDistribution(
    config: AdvancedPlanConfig,
    methodology: TrainingMethodology,
  ) {
    const violations: PrincipleViolation[] = [];
    ...
    if (intensityConfig) {
      ...
      if (easyPercent < expectedEasy - 20) {
        violations.push({
          ...
          mitigation: [
            "Increase easy running percentage",
            "Reduce hard training",
          ],
        });
  ...
  private checkVolumeProgression(
    config: AdvancedPlanConfig,
    methodology: TrainingMethodology,
  ) {
    return {
  ...
  private checkWorkoutPriorities(
    config: AdvancedPlanConfig,
    methodology: TrainingMethodology,
  ) {
    return {
  ...
  private calculatePreservationScore(
    principles: PreservedPrinciple[],
    violations: PrincipleViolation[],
  ): number {
    const avgPreservation =
      principles.reduce((sum, p) => sum + p.preservationLevel, 0) /
      principles.length;
  ...
  private determineIntegrityLevel(
    score: number,
    violations: PrincipleViolation[],
  ): "high" | "medium" | "low" | "compromised" {
    ...
    return {
      allowedModifications: [
        ...
        },
        {
          ...
          rationale:
            "Minor intensity adjustments preserving methodology principles",
        },
      ],
      restrictions: [
        {
          ...
          methodologyPrinciple:
            methodologyConstants.name + " intensity distribution",
        },
      ],
      guidelines: [
        {
          ...
          examples: [
            ...
            "Reduce volume during high stress",
          ],
          warnings: [
  ...
  private generateRecommendations(
    ...
    if (safety.recommendedAction === "abort") {
      recommendations.push(
        "CRITICAL: Do not proceed with current configuration",
      );
    ...
    if (conflicts.length === 0) {
      recommendations.push(
        "Configuration appears safe and methodology-compliant",
      );
    } else {
