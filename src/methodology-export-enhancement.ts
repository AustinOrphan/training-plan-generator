...
import { ExportResult, FormatOptions, ExportMetadata, Formatter } from './export';
...
export class MethodologyExportEnhancer {
  private static philosophyPrinciples: Record<TrainingMethodology, PhilosophyPrinciples> = {
    daniels: {
  ...
  private static researchCitations: Record<TrainingMethodology, ResearchCitation[]> = {
    daniels: [
      {
  ...
  static extractMethodologyInfo(plan: TrainingPlan): {
    ...
    const principles = MethodologyExportEnhancer.philosophyPrinciples[methodology];
  ...
  static generateMethodologyMetadata(
    ...
    const baseMetadata = {
      ...
      planDuration: Math.ceil((plan.config.endDate?.getTime() || Date.now() - plan.config.startDate.getTime()) / (1000 * 60 * 60 * 24 * 7)),
      fileSize: size,
      version: '1.0.0'
    };
    ...
    if (methodology && philosophy && principles) {
      return {
        ...
        researchCitations: MethodologyExportEnhancer.researchCitations[methodology],
  ...
  static generateWorkoutRationale(
    ...
    switch (methodology) {
      case 'daniels':
        rationale = MethodologyExportEnhancer.generateDanielsRationale(workoutType, phase);
        break;
      case 'lydiard':
        rationale = MethodologyExportEnhancer.generateLydiardRationale(workoutType, phase);
        break;
      case 'pfitzinger':
        rationale = MethodologyExportEnhancer.generatePfitzingerRationale(workoutType, phase);
        break;
      case 'hudson':
        rationale = MethodologyExportEnhancer.generateHudsonRationale(workoutType, phase);
        break;
      default:
  ...
  private static generateDanielsRationale(workoutType: string, phase: TrainingPhase): string {
  ...
  private static generateLydiardRationale(workoutType: string, phase: TrainingPhase): string {
    const rationales: Record<string, Record<TrainingPhase, string>> = {
      easy: {
  ...
  private static generatePfitzingerRationale(workoutType: string, phase: TrainingPhase): string {
    const rationales: Record<string, Record<TrainingPhase, string>> = {
      threshold: {
  ...
  private static generateHudsonRationale(workoutType: string, phase: TrainingPhase): string {
    const rationales: Record<string, Record<TrainingPhase, string>> = {
      easy: {
    ...
    return rationales[workoutType]?.[phase] || 
  ...
  static generateMethodologyComparison(
    ...
    const otherMethodologies = Object.keys(TRAINING_METHODOLOGIES)
      .filter(m => m !== currentMethodology) as TrainingMethodology[];
...
export abstract class MethodologyAwareFormatter implements Formatter {
  ...
  abstract formatPlan(plan: TrainingPlan, options?: MethodologyFormatOptions): Promise<MethodologyExportResult>;

  validatePlan(plan: TrainingPlan): { isValid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
  ...
  protected generateEnhancedMetadata(
    ...
    return MethodologyExportEnhancer.generateMethodologyMetadata(plan, format, content);
  }
}
...
export class EnhancedMethodologyJSONFormatter extends MethodologyAwareFormatter {
  ...
  async formatPlan(plan: TrainingPlan, options: MethodologyFormatOptions = {}): Promise<MethodologyExportResult> {
    ...
    const enhancedData = {
      plan: {
        ...
        totalWeeks: Math.ceil((plan.config.endDate?.getTime() || Date.now() - plan.config.startDate.getTime()) / (1000 * 60 * 60 * 24 * 7)),
        totalWorkouts: plan.workouts.length
      },
      methodology: methodology ? {
        ...
        philosophyName: philosophy?.name,
        intensityDistribution: philosophy?.intensityDistribution,
        principles: options.includePhilosophyPrinciples !== false ? principles : undefined,
        citations: options.includeResearchCitations !== false ? citations : undefined
      } : undefined,
      workouts: plan.workouts.map(workout => ({
        ...workout,
        rationale: options.includeWorkoutRationale && methodology ? 
...
export class MethodologyMarkdownFormatter extends MethodologyAwareFormatter {
  ...
  async formatPlan(plan: TrainingPlan, options: MethodologyFormatOptions = {}): Promise<MethodologyExportResult> {
    ...
    plan.workouts.forEach(workout => {
      ...
      if (!currentWeekStart || workoutDate.getTime() - currentWeekStart.getTime() >= 7 * 24 * 60 * 60 * 1000) {
