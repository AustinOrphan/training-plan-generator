...
export class SmartAdaptationEngine implements AdaptationEngine {
  ...
  analyzeProgress(
    ...
    const intensityDistribution =
      this.analyzeIntensityDistribution(completedWorkouts);
  ...
  suggestModifications(
    ...
    if (recovery) {
      ...
      if (
        extendedRecovery.injuryStatus &&
        extendedRecovery.injuryStatus !== "healthy"
      ) {
        modifications.push({
          ...
          suggestedChanges: {
            ...
            volumeReduction:
              extendedRecovery.injuryStatus === "severe" ? 100 : 50,
          },
        });
      }
    }
  ...
  needsAdaptation(progress: ProgressData, recovery?: RecoveryMetrics): boolean {
    ...
    if (
      trainingLoad.ratio > this.SAFE_ACWR_UPPER ||
      trainingLoad.ratio < this.SAFE_ACWR_LOWER
    ) {
      return true;
    }
  ...
  private analyzePerformanceTrend(
    ...
    const olderAvgRelativePace =
      this.calculateAverageRelativePace(olderWorkouts);
    const recentAvgRelativePace =
      this.calculateAverageRelativePace(recentWorkouts);

    const improvement =
      ((olderAvgRelativePace - recentAvgRelativePace) / olderAvgRelativePace) *
      100;
  ...
  private calculateAverageRelativePace(workouts: CompletedWorkout[]): number {
    ...
    return (
      relativePaces.reduce((sum, pace) => sum + pace, 0) / relativePaces.length
    );
  }
  ...
  detectFatigueAndAdjust(
    ...
    if (
      chronicFatigue.days >= this.CHRONIC_FATIGUE_DAYS ||
      tssOverload.consecutive >= 3
    ) {
      fatigueLevel = "severe";
  ...
  assessOverreachingRisk(
    ...
    const recentWeekMileage = runData
      .filter(
        (run) => run.date > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
      )
      .reduce((sum, run) => sum + run.distance, 0);

    const weeklyLoadIncrease =
  ...
  private calculateAcuteFatigue(completedWorkouts: CompletedWorkout[]): number {
    ...
    recentWorkouts.forEach((workout) => {
      ...
      if (
        workout.notes?.toLowerCase().includes("tired") ||
  ...
  private detectChronicFatigue(completedWorkouts: CompletedWorkout[]): {
    ...
    sortedWorkouts.forEach((workout, index) => {
      const highEffort =
        workout.perceivedEffort && workout.perceivedEffort >= 7;
      const plannedDuration = workout.plannedWorkout.targetMetrics.duration;
  ...
  private detectTSSOverload(completedWorkouts: CompletedWorkout[]): {
    ...
    const sortedDays = Array.from(dailyTSS.entries()).sort((a, b) =>
  ...
  private estimateWorkoutTSS(workout: CompletedWorkout): number {
    ...
    const tss =
      (workout.actualDuration * Math.pow(intensityFactor, 2) * 100) / 60;
  ...
  private projectFutureRisk(
    ...
    const recentHighIntensity = completed
      ...
      .filter((w) => w.perceivedEffort && w.perceivedEffort >= 8).length;
      ...
      if (workout.date > new Date()) {
        return {
          ...
          targetMetrics: {
            ...workout.targetMetrics,
            distance: workout.targetMetrics.distance
              ? workout.targetMetrics.distance * factor
              : undefined,
  ...
  private addRecoveryDays(
    ...
    const additionalDays =
      modification.suggestedChanges.additionalRecoveryDays || 2;
    const futureWorkouts = plan.workouts.filter((w) => w.date > new Date());

    // Convert next N hard workouts to recovery
    let converted = 0;
    const modifiedWorkouts: PlannedWorkout[] = plan.workouts.map((workout) => {
      if (
  ...
  private substituteWorkouts(
    ...
    const substituteType =
      modification.suggestedChanges.substituteWorkoutType || "easy";
    const workoutIds = modification.workoutIds || [];

    const modifiedWorkouts = plan.workouts.map((workout) => {
      if (
        workoutIds.includes(workout.id) ||
        (workoutIds.length === 0 && workout.date > new Date())
      ) {
        return {
          ...workout,
          type: substituteType,
  ...
  private applyInjuryProtocol(
    ...
    const volumeReduction =
      modification.suggestedChanges.volumeReduction || 100;
  ...
  private selectAppropriateTemplate(
    ...
    if (
      targetDuration < 45 &&
      template.segments.reduce((sum, s) => sum + s.duration, 0) > 60
    ) {
      // Scale down workout duration
      const scaleFactor =
        targetDuration /
        template.segments.reduce((sum, s) => sum + s.duration, 0);
      return {
        ...template,
...
export type { ProgressData } from "./types";
