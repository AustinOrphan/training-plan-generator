...
import { PROGRESSION_RATES, RECOVERY_MULTIPLIERS, PHASE_DURATION } from './constants';
...
export class WorkoutProgressionSystem {
    ...
    if (!baseTemplate) {
      ...
      const customResult = this.customGenerator.generateWorkout({
        ...
        constraints: {
          ...
          maxIntensity: recoveryState === 'low' ? 70 : recoveryState === 'medium' ? 85 : undefined
        }
      });
    ...
    const rules: ProgressionRule[] = [
      ...
      // Lydiard methodology progression rules
      {
        ...
        parameters: {
          ...
          maxIncrease: 0.30,  // Can increase significantly
        },
        phaseModifiers: {
      ...
      },
      {
        ...
        progressionType: 'stepped',
        parameters: {
          baseIncrease: 0.20, // More aggressive progression
          maxIncrease: 0.50,
   ...
   */
  private calculateProgressedRecoveryTime(type: WorkoutType, segments: WorkoutSegment[]): number {
    const totalDuration = segments.reduce((sum, seg) => sum + seg.duration, 0);
    const avgIntensity = segments.reduce((sum, seg) => sum + seg.intensity * seg.duration, 0) / totalDuration;
    
    const baseRecovery = RECOVERY_MULTIPLIERS[type as keyof typeof RECOVERY_MULTIPLIERS] || 1.0;
   ...
   */
  private updateProgressionDescription(baseDescription: string, multiplier: number): string {
    if (multiplier > 1.1) {
   ...
   */
  private adjustWorkoutIntensity(workout: Workout, adjustment: number): Workout {
  ...
  private buildSubstitutionRationale(
    ...
    return `Substituted ${originalType} with ${newType} due to ${recoveryState} recovery state. ` +
   ...
   */
  private getPhaseAppropriateWorkouts(phase: TrainingPhase, methodology: TrainingMethodology): WorkoutType[] {
