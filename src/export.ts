...
> extends BaseExportOptions<TCustomFields> {
  // FormatOptions can extend BaseExportOptions with additional properties if needed
}
...
export interface ExportManager {
   ...
   */
  registerFormatter<TOptions extends BaseExportOptions>(
...
export class MultiFormatExporter implements ExportManager {
  ...
  registerFormatter<TOptions extends BaseExportOptions>(
    ...
    this.formatters.set(
      formatter.format,
      formatter as Formatter<BaseExportOptions>,
    );
  }
  ...
  private async exportWithMethodologyAwareFormatter(
    ...
    const methodologyOptions: MethodologyFormatOptions = {
      ...
      includeMethodologyComparison:
        options?.includeMethodologyComparison ?? false,
      includeTrainingZoneExplanations:
        options?.includeTrainingZoneExplanations ?? true,
...
> implements Formatter<TOptions>
{
  ...
  getOptionsSchema(): TypedSchema<TOptions> {
    return {
      validate: (data: unknown): TypedResult<TOptions, TypeValidationError> => {
        const isValid = typeof data === "object" && data !== null;
        if (isValid) {
          return { success: true, data: data as TOptions };
        } else {
          return {
            success: false,
  ...
  protected generateMetadata(
    ...
    return {
      ...
      planDuration: plan.config.targetDate
        ? Math.ceil(
            (plan.config.targetDate.getTime() -
              plan.config.startDate.getTime()) /
              (7 * 24 * 60 * 60 * 1000),
          )
        : 16,
  ...
  private generateWeeklyVolumeData(
    plan: TrainingPlan,
  ): { week: number; distance: number }[] {
    const weeklyData = new Map<number, number>();

    plan.workouts.forEach((workout) => {
      const weekNumber =
        Math.floor(
          (workout.date.getTime() - plan.config.startDate.getTime()) /
            (7 * 24 * 60 * 60 * 1000),
        ) + 1;
  ...
  private generatePeriodizationData(
    plan: TrainingPlan,
  ): { phase: string; weeks: number; focus: string[] }[] {
...
class JSONFormatter extends BaseFormatter<JSONOptions> {
  ...
  async formatPlan(
    ...
    const exportData = {
      ...
      charts: (options as JSONOptions)?.includeSchema
        ? this.generateChartData(plan)
        : undefined,
    };
...
class CSVFormatter extends BaseFormatter<CSVOptions> {
  ...
  private generatePlanOverviewSection(plan: TrainingPlan): string {
    ...
    const duration = plan.config.targetDate
      ? Math.ceil(
          (plan.config.targetDate.getTime() - plan.config.startDate.getTime()) /
            (7 * 24 * 60 * 60 * 1000),
        )
      : 16;
    ...
    const rows = [
      ...
      [
        '"End Date"',
        `"${formatDate(plan.config.targetDate || addDays(plan.config.startDate, duration * 7), "yyyy-MM-dd")}"`,
      ],
  ...
  private generateWorkoutScheduleSection(
    ...
    const rows = plan.workouts.map((workout) => {
      ...
      const paceRange = zone
        ? this.calculatePace(zone, 5.0)
        : { min: "", max: "" };
  ...
  private generateTrainingLoadSection(plan: TrainingPlan): string {
    ...
    const rows = weeklyData.map((week, index) => {
      ...
      const avgTSS =
        recentWeeks.reduce((sum, w) => sum + w.plannedTSS, 0) /
        recentWeeks.length;
      const rampRate =
        index > 0 ? ((week.plannedTSS - avgTSS) / avgTSS) * 100 : 0;
      ...
      const acuteTSS =
        acuteWeeks.reduce((sum, w) => sum + w.plannedTSS, 0) /
        Math.min(1, acuteWeeks.length);
      const chronicTSS =
        chronicWeeks.reduce((sum, w) => sum + w.plannedTSS, 0) /
        Math.min(4, chronicWeeks.length);
  ...
  private generatePhaseAnalysisSection(plan: TrainingPlan): string {
    ...
    const rows = plan.blocks.map((block) => {
      ...
      const volumeEmphasis =
        totalDistance / block.weeks > 50
          ? "High"
          : totalDistance / block.weeks > 30
            ? "Moderate"
            : "Low";
      const intensityEmphasis =
        hardWorkouts / totalWorkouts > 0.3
          ? "High"
          : hardWorkouts / totalWorkouts > 0.15
            ? "Moderate"
            : "Low";
    ...
    return Array.from(weeks.entries())
      .map(([weekNumber, workouts]) => {
        ...
        const block = plan.blocks.find(
          (b) =>
            workouts[0] &&
            workouts[0].date >= b.startDate &&
            workouts[0].date <= b.endDate,
        );
        ...
        const moderateCount = workouts.filter(
          (w) =>
            w.targetMetrics.intensity >= 70 && w.targetMetrics.intensity < 85,
        ).length;
...
class iCalFormatter extends BaseFormatter<iCalOptions> {
  ...
  private generateWorkoutEvent(
    ...
    const endTime = new Date(
      startTime.getTime() + workout.targetMetrics.duration * 60000,
    );
    ...
    const eventLines = [
      ...
      workout.targetMetrics.distance
        ? `X-TARGET-DISTANCE:${workout.targetMetrics.distance}`
        : "",
  ...
  private calculateWorkoutStartTime(
    ...
    const optimalTimes: Record<WorkoutType, { hour: number; minute: number }> =
      {
  ...
  private generateWorkoutDescription(
    ...
    const descriptionParts = [
      ...
      workout.targetMetrics.distance
        ? `• Distance: ${workout.targetMetrics.distance} km`
        : "",
      ...
      zone?.heartRateRange
        ? `• Heart Rate: ${zone.heartRateRange.min}-${zone.heartRateRange.max}% Max HR`
        : "",
      paceRange.min !== "0:00"
  ...
  private generateWorkoutLocation(
    ...
    return (
      (options as iCalOptions)?.defaultLocation ||
      "Your Preferred Running Route"
    );
  }
  ...
  private generatePlanOverviewEvent(
    ...
    const description = [
      ...
      "",
      `📅 Duration: ${formatDate(startDate, "MMM dd, yyyy")} - ${formatDate(endDate, "MMM dd, yyyy")}`,
      `🎯 Goal: ${plan.config.goal}`,
    ...
    const eventLines = [
      ...
      `DTSTAMP:${this.formatICalDate(now)}`,
      timeZone === "UTC"
        ? `DTSTART;VALUE=DATE:${formatDate(startDate, "yyyyMMdd")}`
        : `DTSTART;TZID=${timeZone};VALUE=DATE:${formatDate(startDate, "yyyyMMdd")}`,
      timeZone === "UTC"
        ? `DTEND;VALUE=DATE:${formatDate(addDays(endDate, 1), "yyyyMMdd")}`
        : `DTEND;TZID=${timeZone};VALUE=DATE:${formatDate(addDays(endDate, 1), "yyyyMMdd")}`,
...
class PDFFormatter extends BaseFormatter<PDFOptions> {
    ...
    return `
      <div class="header">
        ...
        <div class="subtitle">
          ...
          ${formatDate(plan.config.targetDate || addDays(plan.config.startDate, duration * 7), "MMMM dd, yyyy")}
        </div>
  ...
  private generatePlanOverview(
    ...
    const avgWeeklyDistance =
      chartData.weeklyVolume.length > 0
        ? Math.round(
      ...
      <div class="section">
        ...
        <div class="overview-grid">
          ...
          <div class="overview-card">
            ...
            ${chartData.intensityDistribution
              .map(
                (zone) => `
              <div class="stat-item">
          ...
          <tbody>
            ${workouts
              ...
              .map((workout) => {
                ...
                const paceRange = zone
                  ? this.calculatePace(zone, thresholdPace)
                  : { min: "5:00", max: "5:00" };
                const zoneClass = zone
                  ? `zone-${zone.name.toLowerCase().replace(/\s+/g, "-")}`
                  : "";
            ...
            <div style="margin-bottom: 20px; padding: 15px; background: #f8f9fa; border-radius: 6px;">
              ...
              ${
                ...
                <div style="font-size: 9pt; color: #555;">
                  <strong>Structure:</strong> ${template.segments
  ...
  private generateFooter(): string {
    return `
      <div class="footer">
        ...
        <div>Generated on ${formatDate(new Date(), "MMMM dd, yyyy")}</div>
 ...
 */
class TrainingPeaksFormatter extends BaseFormatter<BaseExportOptions> {
  ...
  private generateTrainingPeaksFormat(
    ...
    return {
      plan: {
        ...
        startDate: plan.config.startDate.toISOString(),
        endDate:
          plan.config.targetDate?.toISOString() ||
          addDays(plan.config.startDate, 112).toISOString(),
        planType: "running",
      ...
      workouts: plan.workouts.map((workout) =>
        this.formatTrainingPeaksWorkout(workout, plan),
      ),
  ...
  private formatTrainingPeaksPace(pace: number): string {
    const minutes = Math.floor(pace);
    const seconds = Math.round((pace - minutes) * 60);
    return `${minutes}:${seconds.toString().padStart(2, "0")}`;
  }
  ...
  private getWorkoutPriority(workout: PlannedWorkout): "A" | "B" | "C" {
    if (
      ["vo2max", "threshold", "race_pace", "time_trial"].includes(workout.type)
    )
      return "A";
  ...
  private calculatePlanDifficulty(
    ...
    const avgTSS =
      plan.workouts.reduce((sum, w) => sum + w.targetMetrics.tss, 0) /
      plan.workouts.length;
    const highIntensityPercent =
      plan.workouts.filter((w) => w.targetMetrics.intensity >= 85).length /
      plan.workouts.length;
  ...
  private generateTrainingPeaksAnnotations(plan: TrainingPlan) {
    ...
    plan.workouts
      .filter(
        (w) =>
          w.targetMetrics.tss >= 100 ||
          ["vo2max", "threshold", "time_trial"].includes(w.type),
      )
      .forEach((workout) => {
        annotations.push({
          date: workout.date.toISOString().split("T")[0],
...
class StravaFormatter extends BaseFormatter {
  ...
  private generateStravaFormat(plan: TrainingPlan, options?: FormatOptions) {
    return {
      ...
      activities: plan.workouts.map((workout) =>
        this.formatStravaActivity(workout, plan),
      ),
  ...
  private generateStravaActivityName(workout: PlannedWorkout): string {
    ...
    const distance = workout.targetMetrics.distance
      ? ` ${workout.targetMetrics.distance}km`
      : "";
    const intensity =
      workout.targetMetrics.intensity >= 85 ? " (High Intensity)" : "";
  ...
  private estimateAverageHR(zone: TrainingZone): number | null {
    if (zone?.heartRateRange) {
      ...
      return Math.round(
        (((zone.heartRateRange.min + zone.heartRateRange.max) / 2) * maxHR) /
          100,
      );
    }
    return null;
  ...
  private calculateAverageSpeed(workout: PlannedWorkout): number {
    if (workout.targetMetrics.distance && workout.targetMetrics.duration) {
      // Speed in m/s
      return (
        (workout.targetMetrics.distance * 1000) /
        (workout.targetMetrics.duration * 60)
      );
    }
  ...
  private generateSegmentEfforts(workout: PlannedWorkout, template: Workout) {
    ...
    return template.segments
      ...
      .map((seg: WorkoutSegment, index: number) => ({
        ...
        distance: Math.round(
          (workout.targetMetrics.distance || 5) *
            (seg.duration / workout.targetMetrics.duration) *
            1000,
        ),
  ...
  private generateStravaGoals(plan: TrainingPlan) {
    ...
    return [
      ...
      },
      {
        ...
        period: "total",
        target:
          plan.workouts.reduce((sum, w) => sum + w.targetMetrics.duration, 0) *
...
class GarminFormatter extends BaseFormatter {
    ...
    return {
      trainingPlan: {
        ...
        estimatedDurationInWeeks: Math.ceil(
          ((plan.config.targetDate?.getTime() ??
            addDays(plan.config.startDate, 112).getTime()) -
            plan.config.startDate.getTime()) /
            (7 * 24 * 60 * 60 * 1000),
        ),
        startDate: plan.config.startDate.toISOString(),
      ...
      workouts: plan.workouts.map((workout) =>
        this.formatGarminWorkout(workout, plan),
      ),
  ...
  private calculateRestDuration(
    ...
    nextSegment: WorkoutSegment | undefined,
  ): number {
...
class EnhancedJSONFormatter extends BaseFormatter {
  ...
  private generateEnhancedJSON(plan: TrainingPlan, options?: FormatOptions) {
    return {
      ...
      workouts: plan.workouts.map((workout) =>
        this.formatAPIWorkout(workout, plan),
      ),
  ...
  private formatWorkoutStructure(template: Workout) {
    return {
      segments: template.segments.map(
        (segment: WorkoutSegment, index: number) => ({
          order: index + 1,
          duration: segment.duration,
          intensity: segment.intensity,
          zone: {
      ...
      totalDuration: template.segments.reduce(
        (sum: number, seg: WorkoutSegment) => sum + seg.duration,
        0,
      ),
  ...
  private generateAPITags(workout: PlannedWorkout): string[] {
    const tags: string[] = [workout.type];
  ...
  private calculateAPIDifficulty(plan: TrainingPlan): {
    level: string;
    score: number;
  ...
  private generateWeeklyData(plan: TrainingPlan) {
    const weeks = new Map<
      number,
      { distance: number; tss: number; workouts: number }
    >();
        ...
        peakWeek: {
          week:
            weeklyData.findIndex(
              (w) => w.tss === Math.max(...weeklyData.map((w) => w.tss)),
            ) + 1,
