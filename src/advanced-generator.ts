...
import { differenceInWeeks, addWeeks, isAfter, isBefore, addDays } from 'date-fns';
...
export class AdvancedTrainingPlanGenerator extends TrainingPlanGenerator {
   ...
   */
  private createFitnessAssessment(phase: TrainingPhase, week: number): PlannedWorkout {
  ...
  private sortRacesByPriority(races: TargetRace[]): TargetRace[] {
    return [...races]
      .filter(race => race.date && race.date instanceof Date && !isNaN(race.date.getTime()))
      .sort((a, b) => {
  ...
  private calculateTransitionPeriod(
    ...
    const weeksBetween = differenceInWeeks(upcomingRace.date, completedRace.date);
  ...
  private getRecoveryWeeks(distance: RaceDistance, priority: 'A' | 'B' | 'C'): number {
    ...
    const priorityMultiplier = priority === 'A' ? 1.0 : priority === 'B' ? 0.7 : 0.5;
  ...
  private getMinimalPreparationWeeks(distance: RaceDistance, priority: 'A' | 'B' | 'C'): number {
    ...
    const priorityMultiplier = priority === 'A' ? 1.0 : priority === 'B' ? 0.6 : 0.3;
    ...
    races.forEach(race => {
      ...
      const raceWorkout: PlannedWorkout = {
        ...
        workout: {
          ...
          recoveryTime: this.getRecoveryWeeks(race.distance, race.priority) * 168 // hours
        },
        targetMetrics: {
          duration: this.estimateRaceDuration(race.distance),
      ...
      if (race.priority === 'A' || race.priority === 'B') {
        const tuneUpWorkout = this.createRaceTuneUpWorkout(race, addDays(race.date, -3));
   ...
   */
  private createRaceTuneUpWorkout(race: TargetRace, date: Date): PlannedWorkout {
