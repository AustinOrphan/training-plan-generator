 ...
 * ```
 */
export interface MethodologyCache<T extends TrainingPhilosophy = TrainingPhilosophy> {
...
export interface CacheKeyGenerator<T> {
  ...
  generateKey(methodology?: TrainingMethodology, params?: CacheKeyParams): string;
 ...
 */
export interface MethodologyCacheManager<T extends TrainingPhilosophy = TrainingPhilosophy> {
...
export interface CachePersistence<T> {
  ...
  save(cache: Map<string, MethodologyCacheEntry<T>>, path: string): Promise<void>;
...
export interface AdvancedCacheOperations<T extends TrainingPhilosophy> {
  ...
  setBatch(entries: Map<string, { value: T; hash: string; metadata?: MethodologyCacheEntry<T>['metadata'] }>): void;
  ...
  warmUp(predictions: Array<{ key: string; value: T; priority: number }>): Promise<void>;
 ...
 */
export interface CacheSynchronization<T> {
  ...
  subscribe(callback: (operation: string, key: string, value?: T) => void): void;
  
  /** Unsubscribe from cache change notifications */
  unsubscribe(callback: (operation: string, key: string, value?: T) => void): void;
