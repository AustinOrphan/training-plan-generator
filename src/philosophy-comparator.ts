...
export class PhilosophyComparator {
    ...
    return [
      {
        ...
        description:
          "How the methodology balances easy, moderate, and hard training",
        scale: 10,
        weight: 0.2,
      },
      {
  ...
  private initializeMethodologyProfiles(): void {
    ...
    this.methodologyProfiles.set("lydiard", {
      ...
      periodizationApproach:
        "Strict base-anaerobic-coordination-taper progression",
    ...
    this.methodologyProfiles.set("pfitzinger", {
      ...
      periodizationApproach:
        "Lactate threshold focused with medium-long emphasis",
      paceCalculationMethod:
        "LT-based pace derivations with race-specific work",
  ...
  public generateComparisonMatrix(): ComparisonMatrix {
    ...
    const overallRankings = methodologies
      .map((methodology) => {
  ...
  public validateMethodology(
    ...
    if (
      profile.researchBasis.length > 0 &&
  ...
  private scorePeriodizationStructure(profile: MethodologyProfile): number {
    ...
    if (
      profile.periodizationApproach.includes("strict") ||
      profile.periodizationApproach.includes("systematic")
    )
      return 9;
  ...
  private findSimilarities(
    ...
    const audienceOverlap = profile1.targetAudience.filter((audience) =>
      profile2.targetAudience.includes(audience),
    );
  ...
  private findDifferences(
    ...
    if (profile1.strengthTraining !== profile2.strengthTraining) {
      const withStrength = profile1.strengthTraining
        ? profile1.methodology
        : profile2.methodology;
      const withoutStrength = profile1.strengthTraining
        ? profile2.methodology
        : profile1.methodology;
      differences.push(
  ...
  private generateRecommendations(
    ...
    if (
      (profile1.methodology === "daniels" &&
        profile2.methodology === "pfitzinger") ||
      (profile1.methodology === "pfitzinger" &&
        profile2.methodology === "daniels")
    ) {
      recommendations.push("Data-driven runners seeking threshold development");
    }

    if (
      (profile1.methodology === "lydiard" &&
        profile2.methodology === "daniels") ||
  ...
  private validateRecoveryPhilosophy(
    ...
    if (
      profile.methodology === "lydiard" &&
      recoveryWorkouts > workouts.length * 0.15
    ) {
      return {
        isValid: false,
